...
CSynthTrack::NoteOn(int note, SYNPAR *tspar,int spd)
{
	...
	float note1 = (float)note+tspar->osctune+
					(float)tspar->oscfinetune*0.0038962f;

	OSC1Speed=(float)pow(2.0, note1/12.0);

	float note2=(float)note1+
...
{
	// Glide Handler
	float oscglide = 1-(syntp->oscglide/127);
