...
//		static const UCHAR TONEPORTAMENTO = 0x1F;
	...
	public:
		...
		struct EffectFlag
		{
			static const int VIBRATO =			0x00000001;///< ビブラート
			static const int PORTAUP =			0x00000002;///< ポルタメントアップ
			static const int PORTADOWN =		0x00000004;///< ポルタメントダウン
			static const int PORTA2NOTE =		0x00000008;///< トーンポルタメント
			static const int VOLUMESLIDE =		0x00000010;///< ボリュームスライドアップ
			static const int PANSLIDE =			0x00000020;///< パンニングスライド左
			static const int TREMOLO =			0x00000040;///< トレモロ
			static const int ARPEGGIO =			0x00000080;///< アルペジオ
			static const int NOTECUT =			0x00000100;///<  ノートカット
		...
		void Init(){
						m_InstrumentNo = 255;// インストルメント無し
			m_Volume = 0;
			m_ChannelVolume = 128;// 真ん中
			m_Note = 255;// ノート無し
			m_PortamentoSpeed = 0;
		...
		inline void Porta2NoteDestPeriod(const int note,const int layer);///< トーンポルタメントの目標値 Set
		const double Porta2NoteDestPeriod(){return m_Porta2NoteDestPeriod;};///< トーンポルタメントの目標値 Get
		...
		/// グリッサンド get state of grissando is playing? 
		const bool IsGrissando(){return m_bGrissando;};
		/// グリッサンド set state of grissando is playing 
		...
		void VibratoSpeed(const int value){	m_VibratoSpeed = value;};///< ビブラート速度 Set
		const int VibratoSpeed() {return m_VibratoSpeed;};///< ビブラートスピード Get

		void VibratoDepth(const int value){	m_VibratoDepth = value;};///< ビブラート深度 Set
		const int VibratoDepth() {return m_VibratoDepth;};///< ビブラート深度 Get
	...
	private:
		...
		// ビブラート
		int m_VibratoSpeed;///< vibrato speed ビブラートスピード
		int m_VibratoDepth;///< vibrato depth ビブラートデプス
		int m_VibratoType;///< vibrato type ビブラートタイプ
		int m_VibratoPos;///< vibrato position 
		double m_VibratoAmount;///< vibrato amount
		double m_AutoVibratoAmount;///< vibrato amount

		int m_Index;///< インスタンスのインデックス
		...
		float panFactor;///< パンの位置(0.0f - 1.0f) 0.5で真ん中
	...
	public:
		...
		void VoiceInit(const UCHAR note);///< ノートオン時の処理
		void ClearEffectFlags() {m_EffectFlags = 0;};///< エフェクトフラグのクリア
		
		const int TickCounter(){return _tickCounter;};///< Tick(Sample) カウンタ Get
		void TickCounter(const int value){_tickCounter = value;};///< Tick(Sample) カウンタ Set
		...
		void NoteOn();
		void NoteOff();///< ノートオフ
		void NoteOffFast();///< ノートカット？
		

		/// リトリガー
		void Retrigger(const int ticks,const int volumeModifier);
		
		/// リサンプルを行いつつnumSamples分の処理を行い、バッファに書き込む
...
//		void Work(const int numSamples,float * pSampleL,float *pSamlpesR);
		...
		void pSampler(XMSampler * const p){m_pSampler = p;};///< 親へのポインタSet
		XMSampler * const pSampler(){return m_pSampler;};///< 親へのポインタGet

		void NextTick(const int value){m_NextTick = value;};///< 次のTickへのサンプル値Set
		const int NextTick(){ return m_NextTick;};///< 次のTickへのサンプル値 Get
		
		const bool IsPlaying(){ return m_bPlay;};///< 演奏中かどうか Get
		void IsPlaying(const bool value){ m_bPlay = value;};///< 演奏中かどうか Set
	...
	private:
		...
		bool m_bPlay;///< 演奏中かどうか？
		int _instrument;///< Instrument 番号

		XMInstrument *m_pInstrument;

		DWORD m_EffectFlags;/// エフェクトフラグ
		...
		int _tickCounter;///< Tick カウンタ
		int m_NextTick;///< 次のTicks
		...
		int effVal;//? これは今後使用しない
		int effCmd;//? これは今後使用しない

		EnvelopeController m_FilterEnvelope;///< フィルターエンベロープコントローラ
		EnvelopeController m_AmplitudeEnvelope;///< アンプエンベロープコントローラ
		EnvelopeController m_PanEnvelope;///< パンニングエンベロープコントローラ
		EnvelopeController m_PitchEnvelope;///< ピッチエンベロープコントローラ
		
		WaveDataController m_WaveDataController;///< サンプリングデータコントローラ
		...
		XMSampler::Channel* m_pChannel;///< チャンネルへのポインタ 
		XMSampler *m_pSampler;///< サンプラへのポインタ
		int effretMode;
		int effretTicks;
		float effretVol;
		int effOld;
	...
	// MOD Tick エミュレーション用
	void TicksPerRow(const int value){m_TicksPerRow = value;};///< Get 1行あたりのティック数
	const int TicksPerRow(){ return m_TicksPerRow;};///< Set 1行あたりのティック数


	XMSampler::Channel& rChannel(const int index){ return m_Channel[index];};///< Channelオブジェクトへの参照
	Voice& rVoice(const int index) { return _voices[index];};///< チャンネルオブジェクトへの参照
...
private:
	...
	int m_BPM;
	int m_TicksPerRow;///< XMでいうところのスピード
	int m_DeltaTick;///< Tickの変化量
	int m_TickCount;///< 1行のTickCounrer
	bool m_bLinearFreq;
	/// 音色データ Instrument Data
