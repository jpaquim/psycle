...
public:
	...
	public:
		inline const Parameter & parameter(const int & parameter) const throw() { return *parameters[parameter]; }
		const char * const description;
		const char * const name;
		const char * const author;
		const char * const help_caption;
		const int columns;
		Information
		(
			const Type & type,
...
#define PSYCLE__PLUGIN__INSTANCIATOR(typename) \
	extern "C" \
	{ \
		__declspec(dllexport) const Host_Plugin::Information & GetInfo() { return typename::information(); } \
		__declspec(dllexport) Plugin & CreateMachine() { return * new typename; } \
	} \
