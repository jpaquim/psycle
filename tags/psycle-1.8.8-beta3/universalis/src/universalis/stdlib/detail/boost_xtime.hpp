...
#ifndef UNIVERSALIS__STDLIB__DETAIL__BOOST_XTIME__INCLUDED
#define UNIVERSALIS__STDLIB__DETAIL__BOOST_XTIME__INCLUDED
#pragma once

#include <universalis/exception.hpp>
#include <boost/thread/xtime.hpp>
#include "utc_time.hpp" // cannot #include "../date_time.hpp" due to dependency of std::hiresolution_clock on universalis::os::clocks, which itself uses std::this_thread::sleep for unit testing, which itself uses this file
...
namespace universalis { namespace stdlib { namespace detail {
	...
	boost::xtime make_boost_xtime(Elapsed_Time const & elapsed_time) {
		...
		seconds const s(ns.get_count() / (1000 * 1000 * 1000));
		boost::xtime xtime;
	...
	boost::xtime inline make_boost_xtime(utc_time const & t) {
