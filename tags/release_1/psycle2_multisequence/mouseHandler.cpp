	...
	case VMPattern:
		{
			...
			if ( ttm >= _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->SONGTRACKS ) ttm = _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->SONGTRACKS-1;
			else if ( ttm < 0 ) ttm = 0;
			...
			else if ( point.y >= YOFFSET )
			{
				...
				int plines = _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternLines[_ps()];
...
//			else if ( editcur.line < 0 ) editcur.line = 0;
	...
	case VMSequence:
			pParentMain->m_wndView.SetFocus();
			// reset selection block
			seqCursor.selRowBeg	= -1;
			seqCursor.selColBeg = -1;
			seqCursor.selRowEnd = -1;
			seqCursor.selColEnd = -1;
			Repaint(DMPlayback);
...
void CChildView::OnMouseMove( UINT nFlags, CPoint point )
{
	...
	case VMPattern:
		...
		if ((nFlags & MK_LBUTTON) && oldm.track != -1)
		{
			...
			{
				ccm=8;
				if ( ttm >= _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->SONGTRACKS ) // Out of Range
				{	
					ttm = _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->SONGTRACKS-1;
		...
		else if (nFlags == MK_MBUTTON)
		{
			...
			{
				int nlines = _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternLines[_ps()];
			...
			if (abs(point.x - MBStart.x) > (ROWWIDTH))
			{
				...
				if (nPos > tOff)
				{
					if (nPos < 0)
						ntOff= 0;
					else if (nPos > _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->SONGTRACKS - VISTRACKS)
						ntOff = _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->SONGTRACKS - VISTRACKS;
					else
						ntOff=nPos;
					bScrollDetatch=true;
					detatchpoint.track = ntOff+1;
...
//				Repaint();
		...
		case VMPattern: // User is in pattern view mode
			...
			if (( point.y >= YOFFSET ) && (point.x >= XOFFSET))
			{
				...
				const int nl = _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternLines[_pSong->playOrder[editPosition]];
...
void CChildView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if ( viewMode == VMPattern )
	{
		...
		{
			case SB_LINEDOWN:
				if ( lOff<_pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternLines[_ps()]-VISLINES)
				{
					nlOff=lOff+1;
					bScrollDetatch=true;
					detatchpoint.track = ntOff+1;
			...
			case SB_PAGEDOWN:
				if ( lOff<_pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternLines[_ps()]-VISLINES)
				{
					const int nl = _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternLines[_ps()];
					nlOff=lOff+16;
					if (nlOff >= nl)
					{
						nlOff = nl-1;
...
void CChildView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	if ( viewMode == VMPattern )
	{
		switch(nSBCode)
		{
			case SB_LINERIGHT:
			case SB_PAGERIGHT:
				if ( tOff < _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->SONGTRACKS - VISTRACKS)
				{
					ntOff=tOff+1;
					bScrollDetatch=true;
					detatchpoint.track = ntOff+1;
			...
			case SB_THUMBTRACK:
				if (ntOff!=(int)nPos)
				{
					const int nt = _pSong->pSequencer->pSequenceData[_pSong->activeTrack]->SONGTRACKS;
					ntOff=(int)nPos;
					if (ntOff >= nt)
					{
						ntOff = nt-1;
