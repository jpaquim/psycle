...
void CChildView::EnterNote(int note, int velocity, bool bTranspose)
{
	...
	if (Global::pPlayer->_playing&&Global::pConfig->_followSong)
	{
		if(_pSong->_trackArmedCount)
		{
			if (velocity == 0)
			{
				for (int i = 0; i < _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS; i++)
				{
				...
				if (i == _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS)
				{
	...
	else 
	{
		if ((GetKeyState(VK_SHIFT)<0) && (note != cdefTweakM) && (note != cdefTweakE) && (note != cdefTweakS) && (note != cdefMIDICC))
		{
			...
			editcur.track = (ChordModeTrack+ChordModeOffs)%_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS;
...
void CChildView::DeleteCurr()
{
	...
	const int ps = _ps();
	unsigned char * offset = _ptrack(ps);
	int patlines = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps];
...
void CChildView::PrevCol(bool wrap,bool updateDisplay)
{
	...
	if(--editcur.col < 0)
	{
		...
		{
			if ( wrap ) 
				editcur.track = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS-1;
			else 
				editcur.col=0;
		}
		else 
...
void CChildView::NextCol(bool wrap,bool updateDisplay)
{
	...
	if (++editcur.col > 8)
	{
		editcur.col = 0;
		pParentMain->StatusBarIdle();
		if (editcur.track == _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS-1)
		{
			if ( wrap ) 
				editcur.track = 0;
			else 
...
void CChildView::PrevLine(int x, bool wrap,bool updateDisplay)
{
	...
	const int nl = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[_ps()];
...
void CChildView::AdvanceLine(int x,bool wrap,bool updateDisplay)
{
	...
	{
		editcur.track+=1;
		if (editcur.track >= _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS)
		{
			editcur.track=0;
			editcur.line+=1;
		}
...
void CChildView::AdvanceTrack(int x,bool wrap,bool updateDisplay)
{
	...
	if(editcur.track>= _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS)
	{
		...
		else editcur.track=_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS-1;
	}
...
void CChildView::PrevTrack(int x,bool wrap,bool updateDisplay)
{
	...
	if(editcur.track<0)
	{
		if (wrap) editcur.track=_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS-1;
...
void CChildView::patCut()
{
	if(viewMode == VMPattern)
	{
		...
		patBufferLines = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps];
		...
		NewPatternDraw(0,_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS,0,patBufferLines-1);
		Repaint(DMData);
	}
}
...
void CChildView::patPaste()
{
	...
	if(patBufferCopy && viewMode == VMPattern)
	{
		...
		AddUndo(ps,0,0,MAX_TRACKS,_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps],editcur.track,editcur.line,editcur.col,editPosition);
		if ( patBufferLines != _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps] )
		{
			AddUndoLength(ps,_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps],editcur.track,editcur.line,editcur.col,editPosition);
...
void CChildView::patTranspose(int trp)
{
	...
	if(viewMode == VMPattern)
	{
		int pLines=_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps];
		...
		NewPatternDraw(0,_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS,editcur.line,pLines-1);
...
void CChildView::PasteBlock(int tx,int lx,bool mix)
{
	...
	{
		int ps = editPosition;
		const int nl = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps];
		...
		for (int t=tx;t<tx+blockNTracks;t++)
		{
			...
			{
				if(l<nl && t<_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS)
				{
...
void CChildView::SaveBlock(FILE* file)
{
	...
	int ps = _ps();
	int nlines = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps];

	fwrite(&_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS, sizeof(int), 1, file);
	fwrite(&nlines, sizeof(int), 1, file);

	for (int t=0;t<_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS;t++)
	{
...
void CChildView::LoadBlock(FILE* file)
{
	...
	if ((nt > 0) && (nl > 0))
	{
		...
		if (nlines != nl)
		{
			...
			_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps] = nl;
		}
...
///////////////////////////////////////////////////////// Add ROW
	...
	{
		st=0;		
		et=_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS;		
		sl=0;
		nl= _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps]/2;	
		el=_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps]-1;
...
///////////////////////////////////////////////////////// Add ROW
	...
	else 
	{
		...
		et=_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS;		
		sl=0;
		nl=_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps];	
		el=_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[ps]/2;
...
void CChildView::BlockTranspose(int trp)
{
	...
	if ( blockSelected == true ) 
	{
		...
		AddUndo(ps,blockSel.start.track,blockSel.start.line,blockSel.end.track-blockSel.start.track+1,
			blockSel.end.line-blockSel.start.line+1,editcur.track,editcur.line,editcur.col,editPosition);
...
//			editPosition = 0;
			...
			if (_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[editPosition]!=Global::pConfig->defaultPatLines)
			{
				AddUndoLength(editPosition,_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[editPosition],editcur.track,editcur.line,editcur.col,editPosition);
				_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[editPosition]=Global::pConfig->defaultPatLines;
			}
...
void CChildView::AddUndo(int pattern, int x, int y, int tracks, int lines, int edittrack, int editline, int editcol, int seqpos, BOOL bWipeRedo, int counter)
{
	...
	pNew->x = x;
	pNew->y = y;
	if (tracks+x > _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS)
	{
		tracks = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS-x;
	}
	pNew->tracks = tracks;
	const int nl = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[pattern];
	if (lines+y > nl)
	{
		lines = nl-y;
	}
...
void CChildView::AddUndoSong(int edittrack, int editline, int editcol, int seqpos, BOOL bWipeRedo, int counter)
{
	...
	{
		if (_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->ppPatternData[i])
		{
			count++;
		}
	}
	...
	for (i = 0; i < MAX_PATTERNS; i++)
	{
		if (_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->ppPatternData[i])
		{
			...
			pWrite+=sizeof(i);
			memcpy(pWrite, _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->ppPatternData[i], MULTIPLY2);
			pWrite+=MULTIPLY2;
		}
	}
...
void CChildView::OnEditUndo() 
{
	...
	if (pUndoList)
	{
		...
		case UNDO_LENGTH:
			if(viewMode == VMPattern)// && bEditMode)
			{
				AddRedoLength(pUndoList->pattern,_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[pUndoList->pattern],editcur.track,editcur.line,editcur.col,pUndoList->seqpos,pUndoList->counter);
				// do undo
				_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[pUndoList->pattern]=pUndoList->lines;
				// set up cursor
				editcur.track = pUndoList->edittrack;
				editcur.line = pUndoList->editline;
				editcur.col = pUndoList->editcol;
...
void CChildView::OnEditRedo() 
{
	...
	if (pRedoList)
	{
		...
		case UNDO_LENGTH:
			if(viewMode == VMPattern)// && bEditMode)
			{
				AddUndoLength(pRedoList->pattern,_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[pUndoList->pattern],editcur.track,editcur.line,editcur.col,pRedoList->seqpos,FALSE,pRedoList->counter);
				// do undo
				_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[pRedoList->pattern]=pRedoList->lines;
				// set up cursor
				editcur.track = pRedoList->edittrack;
				editcur.line = pRedoList->editline;
				editcur.col = pRedoList->editcol;
...
{
	int i;
	for (i = editcur.track+1; i < _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->SONGTRACKS; i++)
	{
		if (_pSong->_trackArmed[i])
		{
