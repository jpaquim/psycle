...
//#define universalis__operating_system__clocks__version__2
...
std::nanoseconds monotonic_wall::current() {
	std::nanoseconds result;
	opaque_time::underlying_type & u(result);
	#if defined DIVERSALIS__OPERATING_SYSTEM__MICROSOFT
		::LARGE_INTEGER counter, frequency;
		if(::QueryPerformanceCounter(&counter) && ::QueryPerformanceFrequency(&frequency)) {
			result = counter.QuadPart * (1000000000 / frequency.QuadPart);
				...
				// microsoft disregards the the unix/posix epoch time, so we need to apply an offset
		...
		result = t.tv_sec * 1000000000 + tv.tv_nsec;
	#endif
	return result;
}
}}}
