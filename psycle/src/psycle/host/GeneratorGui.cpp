...
void GeneratorGui::UpdateVU(CDC* devc) {
	...
	vu_bg_pixbuf_.SetXY(vol+MachineCoords_.dGeneratorVu.x, MachineCoords_.dGeneratorVu.y);
	vu_bg_pixbuf_.SetSize(MachineCoords_.dGeneratorVu.width-vol, MachineCoords_.sGeneratorVu0.height);
	vu_bg_pixbuf_.SetSource(
		MachineCoords_.sGenerator.x + MachineCoords_.dGeneratorVu.x + vol,
	...
	vu_bg_pixbuf_.Draw(devc,dummy,view());
	if(max > 0) {
		vu_peak_pixbuf_.SetXY(max + MachineCoords_.dGeneratorVu.x, MachineCoords_.dGeneratorVu.y);
		vu_peak_pixbuf_.SetSize(MachineCoords_.sGeneratorVuPeak.width, MachineCoords_.sGeneratorVuPeak.height);
		vu_peak_pixbuf_.SetSource(MachineCoords_.sGeneratorVuPeak.x, MachineCoords_.sGeneratorVuPeak.y);
		vu_peak_pixbuf_.Draw(devc,dummy,view());
	}
	if(vol > 0) {
		vu_led_pixbuf_.SetXY(MachineCoords_.dGeneratorVu.x, MachineCoords_.dGeneratorVu.y);
		vu_led_pixbuf_.SetSize(vol, MachineCoords_.sGeneratorVu0.height);
		vu_led_pixbuf_.SetSource(MachineCoords_.sGeneratorVu0.x, MachineCoords_.sGeneratorVu0.y);
...
void GeneratorGui::UpdatePan() {
	...
	panning /= 128;
	pan_pixbuf_.SetXY(panning + MachineCoords_.dGeneratorPan.x, MachineCoords_.dGeneratorPan.y);
}
...
void GeneratorGui::SetSkin(
	...
	pixbuf_.SetSize(MachineCoords.sGenerator.width, MachineCoords.sGenerator.height);
	pixbuf_.SetSource(MachineCoords.sGenerator.x, MachineCoords.sGenerator.y);
	mute_pixbuf_.SetXY(MachineCoords.dGeneratorMute.x, MachineCoords.dGeneratorMute.y);
	mute_pixbuf_.SetSize(MachineCoords.sGeneratorMute.width, MachineCoords.sGeneratorMute.height);
	mute_pixbuf_.SetSource(MachineCoords.sGeneratorMute.x, MachineCoords.sGeneratorMute.y);
	mute_pixbuf_.SetVisible(false);
	solo_pixbuf_.SetXY(MachineCoords.dGeneratorSolo.x, MachineCoords.dGeneratorSolo.y);
	solo_pixbuf_.SetSize(MachineCoords.sGeneratorSolo.width, MachineCoords.sGeneratorSolo.height);
	solo_pixbuf_.SetSource(MachineCoords.sGeneratorSolo.x, MachineCoords.sGeneratorSolo.y);
	solo_pixbuf_.SetVisible(false);
	UpdatePan();
	pan_pixbuf_.SetSize(MachineCoords.sGeneratorPan.width, MachineCoords.sGeneratorPan.height);
	pan_pixbuf_.SetSource(MachineCoords.sGeneratorPan.x, MachineCoords.sGeneratorPan.y);

	text_.SetXY(MachineCoords.dGeneratorName.x, MachineCoords.dGeneratorName.y);			
	text_.SetFont(font);
	text_.SetColor(
		GetRValue(font_color) / 255.0,
...
bool GeneratorGui::OnEvent(canvas::Event* ev) {
	if(ev->type == canvas::Event::BUTTON_PRESS) {
		view()->SelectMachine(this);
		if(!TestSolo(ev->x, ev->y))
			if(!TestMute(ev->x, ev->y))
				TestPan(ev->x, ev->y);
	} else if(ev->type == canvas::Event::MOTION_NOTIFY) {
		if(pan_dragging_) {
			DoPanDragging(ev->x, ev->y);
			return true;
		}
	} else if( ev->type == canvas::Event::BUTTON_RELEASE) {
		pan_dragging_ = false;
	}
