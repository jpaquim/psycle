...
typedef float (*PRESAMPLERFN)(const short *pData, uint64 offset, uint32 res, uint64 length);
...
class Resampler
{
  ...
  protected:
    ...
    static float T_None(const short *pData, uint64 offset, uint32 res, uint64 length)
    {
      return *pData;
    }
};
