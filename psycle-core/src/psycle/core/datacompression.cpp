...
int DataCompression::BEERZ77Comp2(std::uint8_t * pSource, std::uint8_t ** pDestination, int size)
{
	...
	if (pSource)
	{
		...
		*pDestPos++ = (std::uint8_t)0x04; // file version
		*pDestPos++ = (std::uint8_t)(size & 0xff);
		*pDestPos++ = (std::uint8_t)((size>>8) & 0xff);
		*pDestPos++ = (std::uint8_t)((size>>16) & 0xff);
		*pDestPos++ = (std::uint8_t)((size>>24) & 0xff);

		// we will compress pSource into pDest
		std::uint8_t * pSlidingWindow = pSource;
		std::uint8_t * pCurrentPos = pSource;
		std::uint8_t * pTestPos;
		...
		while (pCurrentPos < pSource+size)
		{
			...
			std::uint8_t * pBestMatch = pCurrentPos;
			...
			for (pTestPos = pSlidingWindow; pTestPos < pCurrentPos-MIN_REDUNDANT_BYTES_2; pTestPos++)
			{
				...
				{
					// set our pointers
					std::uint8_t* pMatchPosEnd = pCurrentPos;
					std::uint8_t* pMatchPosStart;
...
bool DataCompression::BEERZ77Decomp2(std::uint8_t * pSourcePos, std::uint8_t ** pDestination)
{
	// remember to delete your destination when done
	if (pSourcePos) 
	{
...
int DataCompression::SoundSquash(signed short * pSource, std::uint8_t ** pDestination, int size)
{
	if (pSource)
	{
		std::uint8_t * pDestPos = *pDestination = new std::uint8_t[(size*12/4)+5]; // worst case-remember words are 2 bytes

		memset(pDestPos, 0, (size*12/4)+5);
		*pDestPos++ = 0x01; // file version
		*pDestPos++ = (std::uint8_t)(size & 0xff); // size
		*pDestPos++ = (std::uint8_t)((size>>8) & 0xff);
		*pDestPos++ = (std::uint8_t)((size>>16) & 0xff);
		*pDestPos++ = (std::uint8_t)((size>>24) & 0xff);
...
bool DataCompression::SoundDesquash(std::uint8_t * pSourcePos, signed short ** pDestination)
{
