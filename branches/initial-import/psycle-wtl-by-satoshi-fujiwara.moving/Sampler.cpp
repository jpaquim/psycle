...
TCHAR* Sampler::_psName = _T("Sampler");
...
Sampler::Sampler(int index)
{
	...
	_stprintf(_editName, _T("Sampler"));
...
bool Sampler::Load(RiffFile& riffFile)
{
	int i;
	char junk[256];
...
void Sampler::VoiceWork(int numsamples, int voice)
{
	...
	else if ((pVoice->_triggerNoteDelay) && (pVoice->_tickCounter >= pVoice->_triggerNoteDelay))
	{
		if ( pVoice->effCmd == SAMPLER_CMD_RETRIG && pVoice->effretTicks)
		{
			...
			pVoice->_envelope._step = (1.0f/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_AT)*(44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
			pVoice->_filterEnv._step = (1.0f/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_AT)*(44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
...
*/
	...
	{
		if ( Global::_pSong->pInstrument(data._inst)->waveLength[0] == 0 ) return; // if no wave, return.
		...
		for ( voice=0; voice<_numVoices; voice++)
		{
			...
			{
				switch (Global::_pSong->pInstrument(_voices[voice]._instrument)->_NNA)
				{
				case 0:
					NoteOffFast(voice);
					break;
...
//  All this mess should be really changed with classes using the "operator=" to "copy" values.
	...
	int layer = 0; // Change this when adding working Layering code.
	int twlength = Global::_pSong->pInstrument(pEntry->_inst)->waveLength[layer];
	...
	if (pEntry->_note < 120 && twlength > 0)
	{
		...
		if (Global::_pSong->pInstrument(pVoice->_instrument)->_RCUT)
		{
			pVoice->_cutoff = alteRand(Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_CO);
		}
		else
		{
			pVoice->_cutoff = Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_CO;
		}
		
		if (Global::_pSong->pInstrument(pVoice->_instrument)->_RRES)
		{
			pVoice->_filter._q = alteRand(Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_RQ);
		}
		else
		{
			pVoice->_filter._q = Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_RQ;
		}

		pVoice->_filter._type = (FilterType)Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_TP;
		pVoice->_coModify = (float)Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_EA;
		pVoice->_filterEnv._sustain = (float)Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_SL*0.0078125f;

		if (( pEntry->_cmd != SAMPLER_CMD_EXTENDED) || ((pEntry->_parameter & 0xf0) != SAMPLER_CMD_EXT_NOTEDELAY))
		{
			pVoice->_filterEnv._stage = ENV_ATTACK;
		}
		pVoice->_filterEnv._step = 
			(1.0f/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_AT)
			 * (44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
		pVoice->_filterEnv._value = 0;
		
		// Init Wave
		//
		pVoice->_wave._pL = Global::_pSong->pInstrument(pVoice->_instrument)->waveDataL[layer];
		pVoice->_wave._pR = Global::_pSong->pInstrument(pVoice->_instrument)->waveDataR[layer];
		pVoice->_wave._stereo = Global::_pSong->pInstrument(pVoice->_instrument)->waveStereo[layer];
		pVoice->_wave._length = twlength;
		
		// Init loop
		if (Global::_pSong->pInstrument(pVoice->_instrument)->waveLoopType[layer])
		{
			pVoice->_wave._loop = true;
			pVoice->_wave._loopStart = Global::_pSong->pInstrument(pVoice->_instrument)->waveLoopStart[layer];
			pVoice->_wave._loopEnd = Global::_pSong->pInstrument(pVoice->_instrument)->waveLoopEnd[layer];
		}
		else
		{
		...
		// Init Resampler
		//
		if (Global::_pSong->pInstrument(pVoice->_instrument)->_loop)
		{
			double const totalsamples = 
				(double)(Global::_pSong->SamplesPerTick() 
				 * Global::_pSong->pInstrument(pVoice->_instrument)->_lines);
//			pVoice->_wave._speed = (__int64)((pVoice->_wave._length/totalsamples)*4294967296.0f*44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
		...
		{
			float const finetune = CValueMapper::Map_255_1(Global::_pSong->pInstrument(pVoice->_instrument)->waveFinetune[layer]);
#if defined(_WINAMP_PLUGIN_)
			pVoice->_wave._speed = (__int64)(pow(2.0f, ((pEntry->_note+Global::_pSong->pInstrument(pVoice->_instrument)->waveTune[layer])-48 +finetune)/12.0f)*4294967296.0f*(44100.0f/Global::pConfig->_samplesPerSec));
#else
			pVoice->_wave._speed = (__int64)(pow(2.0f, ((pEntry->_note+Global::_pSong->pInstrument(pVoice->_instrument)->waveTune[layer])-48 +finetune)/12.0f)*4294967296.0f*(44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec));
		...
		//
		pVoice->_wave._vol = (float)Global::_pSong->pInstrument(pVoice->_instrument)->waveVolume[layer]*0.01f;
		...
		if (pEntry->_volcmd == PatternCmd::VELOCITY)
		{
			pVoice->_wave._vol *= CValueMapper::Map_255_1(pEntry->_volume);
		}
		// Panning calculation -------------------------------------------
		//
		float panFactor;
		
		if (Global::_pSong->pInstrument(pVoice->_instrument)->_RPAN)
		{
		...
		else if ( pEntry->_cmd == SAMPLER_CMD_PANNING )
		{
			...
			panFactor = CValueMapper::Map_255_1(Global::_pSong->pInstrument(pVoice->_instrument)->_pan);
		}
		...
		//
#if defined(_WINAMP_PLUGIN_)
		pVoice->_envelope._step = (1.0f/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_AT)*(44100.0f/Global::pConfig->_samplesPerSec);
#else
		pVoice->_envelope._step = (1.0f/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_AT)*(44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
#endif // _WINAMP_PLUGIN
		pVoice->_envelope._value = 0.0f;
		pVoice->_envelope._sustain = (float)Global::_pSong->pInstrument(pVoice->_instrument)->ENV_SL*0.01f;
		// This must be last, or the voice could be started by VoiceWork before all
		// elements are initialized
		//
		if (( pEntry->_cmd == SAMPLER_CMD_EXTENDED) && ((pEntry->_parameter & 0xf0) == SAMPLER_CMD_EXT_NOTEDELAY))
		{
			pVoice->_triggerNoteDelay 
				= (Global::_pSong->SamplesPerTick() / 6)*(pEntry->_parameter & 0x0f);
			pVoice->_envelope._stage = ENV_OFF;
		}
		else
		{
			if (pEntry->_cmd == SAMPLER_CMD_RETRIG && (pEntry->_parameter&0x0f) > 0)
			{
				...
				pVoice->effVal 
					= (Global::_pSong->SamplesPerTick() / (pVoice->effretTicks+1));
	...
	else if ((pEntry->_cmd == SAMPLER_CMD_VOLUME) || 
		( pEntry->_cmd == SAMPLER_CMD_PANNING ) || 
		( pEntry->_volcmd != 0))
	{
		// Calculating volume coef ---------------------------------------
		//
	...
	int voice)
{
...
#if defined(_WINAMP_PLUGIN_)
			pVoice->_filterEnv._step = ((1.0f - pVoice->_filterEnv._sustain) / Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_DT) * (44100.0f/Global::pConfig->_samplesPerSec);
#else
			pVoice->_filterEnv._step = ((1.0f - pVoice->_filterEnv._sustain) / Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_DT) * (44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
#endif // _WINAMP_PLUGIN_
		}
		break;
	case ENV_DECAY:
	...
	case ENV_RELEASE:
		...
		if (pVoice->_filterEnv._value < 0.0f)
		{
			pVoice->_filterEnv._value = 0;
...
#if defined(_WINAMP_PLUGIN_)
			pVoice->_envelope._step = ((1.0f - pVoice->_envelope._sustain)/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_DT)*(44100.0f/Global::pConfig->_samplesPerSec);
#else
			pVoice->_envelope._step = ((1.0f - pVoice->_envelope._sustain)/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_DT)*(44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
#endif // _WINAMP_PLUGIN_
		}
		break;
	case ENV_DECAY:
...
#if defined(_WINAMP_PLUGIN_)
		pVoice->_envelope._step = (pVoice->_envelope._value/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_RT)*(44100.0f/Global::pConfig->_samplesPerSec);
		pVoice->_filterEnv._step = (pVoice->_filterEnv._value/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_RT)*(44100.0f/Global::pConfig->_samplesPerSec);
#else
		pVoice->_envelope._step = (pVoice->_envelope._value/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_RT)*(44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
		pVoice->_filterEnv._step = (pVoice->_filterEnv._value/Global::_pSong->pInstrument(pVoice->_instrument)->ENV_F_RT)*(44100.0f/Global::pConfig->_pOutputDriver->_samplesPerSec);
#endif // _WINAMP_PLUGIN_
	}
}
