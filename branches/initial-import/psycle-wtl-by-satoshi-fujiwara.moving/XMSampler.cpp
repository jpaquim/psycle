...
namespace SF {
	...
	XMSampler::XMSampler(int index) 
	{
		...
		for (int i = 0; i < XMSampler::MAX_POLYPHONY; i++)
		{
			_voices[i].Init();
...
//		riffFile.Write(&size,sizeof(size));
		...
		PatternEntry* pData)
	{
		...
		PatternEntry& data = *pData;///< パターンエントリへの参照
		bool _bInstrumentSet = data._inst < 255;///< インストルメントがセットされているか？
		bool _bvoiceFound = false;///<現在このチャンネルで発音中のボイスがあるか？
		bool _bNoteOn = (data._note < 120) & (data._cmd != CMD::NOTE_OFF) & (data._volcmd != CMD::NOTE_OFF);///<ノートオンか？
		bool _bNotPorta2Note = !((data._cmd == CMD::PORTA2NOTE) || (data._volcmd == CMD::PORTA2NOTE));
		
		///< 現在このチャンネルで発音しているボイスのインデックス
		int _current;///< 現在このチャンネルで発音しているボイスのインデックス
		
		XMSampler::Channel& _channel = rChannel(channel);///< チャンネルオブジェクトへの参照
		...
		Voice* _currentVoice;///< 現在このチャンネルで発音しているボイスへのポインタ
		Voice* _newVoice;///< 処理対象のボイスへのポインタ

		// 現在このチャンネルで発音中のボイスを検索する
		...
		{
			int _newVoiceIndex = -1 ;///< 空きボイスへのインデックス
			bool _bFound = false;///< 空きボイスが見つかったか？
		...
		if(_bNoteOn)
		{
			...
			if(twlength > 0){
				...
				if((!_bNotPorta2Note) & (_bvoiceFound))
				{
					...
					_newVoice->VoiceInit(_channel.Note()); // 初期化
					_channel.EffectInit();
...
//		_filterEnv.Value(0);
	...
	void XMSampler::Voice::Work(int numSamples,float * pSamplesL,float * pSamplesR,Cubic& _resampler)
	{
		...
		while (numSamples)
		{
			...
			// MODでいうTickが来たらエフェクトを呼び出す。
			// ?ここはSampleTick単位に変更する
...
*/
			...
			// Monoralの場合、left outputとright outputの値を同じにする
			if(!m_WaveDataController.IsStereo()){
				right_output = left_output;
			}
...
//		::ATLTRACE2(_T("porta2note: value:%x dest:%f curr:%f\n"),m_PortamentoValue,m_Porta2NoteDestSpeed,voice.Wave().Speed() * m_PortamentoAmount);
	...
	const int XMSampler::Channel::PeriodToNote(const double period,const int layer)
	{
		...
		if(m_pSampler->IsLinearFreq()){
			...
			// note = (int)(10.0 * 12.0  - (double)_wave.WaveTune() - _wave.WaveFineTune() / 256.0 - period / 64.0 + 0.5/* 四捨五入*/);
	...
	/// XMのTempoとSpeedから実際のTempoおよびdelta tickを計算する
