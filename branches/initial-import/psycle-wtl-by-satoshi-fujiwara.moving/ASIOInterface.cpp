 ...
 *  asio drivers will tell us their preferred settings with : ASIOGetBufferSize
...
AudioDriverInfo ASIOInterface::_info = { _T("ASIO 2.0 Output") };
CComAutoCriticalSection ASIOInterface::_lock;

ASIOInterface::ASIOInterface(iController * pController) : AudioDriver(pController)
{
	_initialized = false;
	_configured = false;
	_running = false;
//	_lock.Init();
	...
	TCHAR* pNameBuf[MAX_ASIO_DRIVERS];
	TCHAR _szNameBuf[MAX_ASIO_DRIVERS][NAME_LEN];
	
	::ZeroMemory(_szNameBuf,sizeof(_szNameBuf));

	for (UINT i = 0; i < MAX_ASIO_DRIVERS; i++)
	{
		pNameBuf[i] = _szNameBuf[i];
		driverindex[i] = 0;
		outputindex[i] = 0;
		minSamples[i] = 0;
		maxSamples[i] = 0;
	...
	int drivers = asioDrivers.getDriverNames((TCHAR**)pNameBuf,NAME_LEN);
	drivercount = 0;
	if(drivers > 0){
		for (int i = 0; i < MAX_ASIO_DRIVERS; i++)
		{
			_tcscpy(szNameBuf[i],_szNameBuf[i]);
		}
		...
		for (i = 0; i < drivers; i++)
		{
			...
			if (_tcscmp(SF::CResourceString(IDS_MSG0005),szNameBuf[i])==0)
			{
				continue;
			}
			if (_tcscmp(SF::CResourceString(IDS_MSG0006),szNameBuf[i])==0)
			{
				continue;
			}
			if (_tcscmp(SF::CResourceString(IDS_MSG0007),szNameBuf[i])==0)
			{
				continue;
			}
	#endif
			...
			//		TRACE(szNameBuf[i]);
					...
					if (ASIOGetChannels(&in,&out) == ASE_OK)
					{
						for (int j = 0; j < out; j+=2)
						{
							...
							_stprintf(szFullName[drivercount],_T("%s : %s"),szNameBuf[i],CA2T(channelInfo.name));
							...
							case ASIOSTInt32MSB16:		// 32 bit data with 18 bit alignment
								_tcscat(szFullName[drivercount],_T(" : 16 bit"));
								break;
							case ASIOSTInt32LSB18:		// 32 bit data with 18 bit alignment
							case ASIOSTInt32MSB18:		// 32 bit data with 18 bit alignment
								_tcscat(szFullName[drivercount],_T(" : 18 bit"));
								break;

							case ASIOSTInt32LSB20:		// 32 bit data with 20 bit alignment
							case ASIOSTInt32MSB20:		// 32 bit data with 20 bit alignment
								_tcscat(szFullName[drivercount],_T(" : 20 bit"));
								break;
							case ASIOSTInt24LSB:		// used for 20 bits as well
							case ASIOSTInt32LSB24:		// 32 bit data with 24 bit alignment
							case ASIOSTInt24MSB:		// used for 20 bits as well
							case ASIOSTInt32MSB24:		// 32 bit data with 24 bit alignment
								_tcscat(szFullName[drivercount],_T(" : 24 bit"));
								break;
							case ASIOSTInt32LSB:
							case ASIOSTInt32MSB:
								_tcscat(szFullName[drivercount],_T(": 32 bit"));
								break;
							case ASIOSTFloat32LSB:		// IEEE 754 32 bit float, as found on Intel x86 architecture
								_tcscat(szFullName[drivercount],_T(": 32 bit float"));
								break;
							case ASIOSTFloat64LSB: 		// IEEE 754 64 bit double float, as found on Intel x86 architecture
								_tcscat(szFullName[drivercount],_T(": 64 bit float"));
								break;
							case ASIOSTFloat32MSB:		// IEEE 754 32 bit float, as found on Intel x86 architecture
							case ASIOSTFloat64MSB: 		// IEEE 754 64 bit double float, as found on Intel x86 architecture
								_tcscat(szFullName[drivercount],_T(" : unsupported MSB float bitorder"));
								continue;
								break;
							}
							drivercount++;
...
void ASIOInterface::Error(TCHAR const *msg)
{
	MessageBox(NULL, msg, SF::CResourceString(IDS_MSG0008), MB_ICONWARNING);
}
...
void ASIOInterface::Initialize(
	...
	const void* context)
{
	_pASIOcallbackContext = const_cast<void*>(context);
...
void ASIOInterface::ReadConfig()
{
	...
	if (reg.OpenKey(_T("ASIOOut")) != ERROR_SUCCESS)
	{
		return;
	}
	...
	configured &= (reg.QueryValue(_T("BufferSize"), &type, (BYTE*)&_ASIObufferSize, &numData) == ERROR_SUCCESS);
	numData = sizeof(_driverID);
	configured &= (reg.QueryValue(_T("DriverID"), &type, (BYTE*)&_driverID, &numData) == ERROR_SUCCESS);
	numData = sizeof(m_SamplesPerSec);
	configured &= (reg.QueryValue(_T("SamplesPerSec"), &type, (BYTE*)&m_SamplesPerSec, &numData) == ERROR_SUCCESS);
	reg.CloseKey();
	reg.CloseRootKey();
...
void ASIOInterface::WriteConfig()
{
	...
	if (reg.OpenKey(_T("ASIOOut")) != ERROR_SUCCESS)
	{
		if (reg.CreateKey(_T("ASIOOut")) != ERROR_SUCCESS)
		{
			Error(SF::CResourceString(IDS_ERR_MSG0035));
			return;
		}
	}
	reg.SetValue(_T("BufferSize"), REG_DWORD, (BYTE*)&_ASIObufferSize, sizeof(_ASIObufferSize));
	reg.SetValue(_T("DriverID"), REG_DWORD, (BYTE*)&_driverID, sizeof(_driverID));
	reg.SetValue(_T("SamplesPerSec"), REG_DWORD, (BYTE*)&m_SamplesPerSec, sizeof(m_SamplesPerSec));
	reg.CloseKey();
	reg.CloseRootKey();
}
...
const bool ASIOInterface::Start()
{
	
	CComCritSecLock<CComAutoCriticalSection> lock(_lock, true);
	if (_running)
	{
		return true;
	}
...
//		ASIOExit();
	...
	if (ASIOSetSampleRate(m_SamplesPerSec) != ASE_OK)
	{
		m_SamplesPerSec = 44100;
		if (ASIOSetSampleRate(m_SamplesPerSec) != ASE_OK)
		{
...
//		ASIOExit();
	...
	_running = true;
	// TODO  ReSyncコマンドを元に戻す。
	//CMidiInput::Instance()->ReSync();	// MIDI IMPLEMENTATION
	return true;
}

const bool ASIOInterface::Stop()
{
	CComCritSecLock<CComAutoCriticalSection> lock(_lock, true);
	if (!_running)
	{
...
const bool ASIOInterface::Enable(const bool e)
{
	return e ? Start() : Stop();
}

const int ASIOInterface::WritePos() const
{
	// Not yet implemted
	if (!_running)
	{
...
const int ASIOInterface::PlayPos() const
{
	// Not yet implemted
	if (!_running)
	{
...
const int ASIOInterface::BufferSize() const
{ 
	return _ASIObufferSize; 
};

void ASIOInterface::ControlPanel(const int driverID)
{
