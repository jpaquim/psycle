...
int VSTPlugin::Instance(const TCHAR *dllname,bool overwriteName)
{
	_pEffect = NULL;
	instantiated = false;
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_GETPROCADDRESS)) % error).str().c_str(),
			dllname,NULL);
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_CALL_MAIN)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
	...
	{
		ATLTRACE(SF::CResourceString(IDS_TRACE_MSG0000),(int)_pEffect);
		FreeLibrary(h_dll);
		_pEffect = NULL;
		return VSTINSTANCE_ERR_REJECTED;
	}

	ATLTRACE(SF::CResourceString(IDS_TRACE_MSG0001),(int)_pEffect);
...
//2 :     Host to Plug, setSampleRate ( 44100.000000 ) 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0001)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
...
// 3 :     Host to Plug, setBlockSize ( 512 ) 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0002)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
...
//  4 :     Host to Plug, open
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0003)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
...
// 5 :     Host to Plug, setSpeakerArrangement  returned : false 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0004)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
...
// 6 :     Host to Plug, setSampleRate ( 44100.000000 ) 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0005)) % error).str().c_str(),
			dllname,NULL);
...
// 7 :     Host to Plug, setBlockSize ( 512 ) 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0006)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
...
// 8 :     Host to Plug, setSpeakerArrangement  returned : false 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0007)) % error).str().c_str(),
			dllname,NULL);
...
// 9 :     Host to Plug, setSampleRate ( 44100.000000 ) 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0008)) % error).str().c_str(),
			dllname,NULL);
...
// 10 :     Host to Plug, setBlockSize ( 512 ) 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0009)) % error).str().c_str(),
			dllname,NULL);
...
// 11 :     Host to Plug, getProgram  returned : 0 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0010)) % error).str().c_str(),
			dllname,NULL);
...
// 13 :     Host to Plug, getVstVersion  returned : 2300 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0011)) % error).str().c_str(),
			dllname,NULL);
...
// 14 :     Host to Plug, getProgramNameIndexed ( -1 , 0 , ptr to char ) 
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0012)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
	...
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0013)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
	...
	if (!Dispatch( effGetEffectName, 0, 0, _name, 0.0f))
	{
		CString str1(dllname);
		CString str2 = str1.Mid(str1.ReverseFind(L'\\')+1);
		str1 = str2.Left(str2.Find(L'.'));
		_tcscpy(_sProductName,str1);
	} else {
		_tcscpy(_sProductName,CA2T(_name));
	}
	
	if ( overwriteName ) 
	{
		memcpy(_editName,_sProductName,31 * sizeof(TCHAR));
	}
	_editName[31]=L'\0';

// Compatibility hacks
	if ( _tcscmp(_sProductName,_T("sc-101")) == 0 ) 
	{
		requiresRepl = true;
	}
...
//
	...
	try
	{
		if (!_pEffect->dispatcher(_pEffect, effGetVendorString, 0, 0, &_name, 0.0f))
		{
			_tcscpy(_sVendorName, _T("Unknown vendor"));
		} else {
			_tcscpy(_sVendorName,CA2T(_name));
		}
	}
	catch (char *error)
	{
		MessageBox(NULL,
			(SF::tformat(SF::CResourceString(IDS_ERR_MSG0014)) % error).str().c_str(),
			dllname,NULL);
		_pEffect=NULL;
		return VSTINSTANCE_ERR_EXCEPTION;
	}
	...
	_sDllName = new TCHAR[_tcslen(dllname) + 1];
	_stprintf(_sDllName,dllname);

	ATLTRACE(SF::CResourceString(IDS_TRACE_MSG0002),_sDllName);

	instantiated=true;

	ATLTRACE(SF::CResourceString(IDS_TRACE_MSG0003),_pEffect->numInputs,_pEffect->numOutputs);
...
void VSTPlugin::Free() // Called also in destruction
{
	...
	{
		instantiated=false;
		ATLTRACE(SF::CResourceString(IDS_TRACE_MSG0005),(int)_pEffect);
		_pEffect->user = NULL;
		try
		{
		...
		{
			MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0018),_editName,NULL);
		}
		...
		{
			MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0027),_editName,NULL);
		}
...
bool VSTPlugin::Load(RiffFile& riffFile)
{
	char junkdata[256];
...
/*  This part is read when loading the song to detect the machine type.
	...
	riffFile.Read(&_x, sizeof(_x));
	riffFile.Read(&_y, sizeof(_y));
	riffFile.Read(&_type, sizeof(_type));
*/
...
/*  Enable this when changing the File Format.
	...
	char dllName[128];
	riffFile.Read(dllName,sizeof(dllName));
	_strlwr(dllName);
	...
	if ( CNewMachine::dllNames.Lookup(dllName,sPath) ) 
	{
		...
		{
			CString error;
			_stprintf(error,"Error. '%s' is Missing or Corrupted,tempName);
	...
	Init();
	riffFile.Read(&_editName, sizeof(_editName));

	int num;
	riffFile.Read(&num,sizeof(int));

	if ( !instantiated ) 
	{
		for (int p=0;p<num;p++ ) riffFile.Read(&junkdata,sizeof(float));
		riffFile.Read(&junkdata,sizeof(int));

		if(_pEffect->flags & effFlagsProgramChunks)
		{
			long chunk_size;
			riffFile.Read(&chunk_size,sizeof(long));
			
			// Read chunk
			char *chunk=new char[chunk_size];
			riffFile.Read(chunk,chunk_size);
			delete chunk;
		}
	}
	else 
	{
		...
		for(int p=0;p<num;p++)
		{
			riffFile.Write(&value,sizeof(float));
			_pEffect->setParameter(_pEffect,p,value);
		}
		int cprog;
		riffFile.Read(&cprog,sizeof(int));
...
*/
	...
	riffFile.Read(&junkdata[0], 8*sizeof(int)); // SubTrack[]
	riffFile.Read(&junkdata[0], sizeof(int)); // numSubtracks
	riffFile.Read(&junkdata[0], sizeof(int)); // interpol

	riffFile.Read(&junk[0], sizeof(int)); // outwet
	riffFile.Read(&junk[0], sizeof(int)); // outdry

	riffFile.Read(&junkdata[0], sizeof(int)); // distPosThreshold
	riffFile.Read(&junkdata[0], sizeof(int)); // distPosClamp
	riffFile.Read(&junkdata[0], sizeof(int)); // distNegThreshold
	riffFile.Read(&junkdata[0], sizeof(int)); // distNegClamp

	riffFile.Read(&junkdata[0], sizeof(char)); // sinespeed
	riffFile.Read(&junkdata[0], sizeof(char)); // sineglide
	riffFile.Read(&junkdata[0], sizeof(char)); // sinevolume
	riffFile.Read(&junkdata[0], sizeof(char)); // sinelfospeed
	riffFile.Read(&junkdata[0], sizeof(char)); // sinelfoamp

	riffFile.Read(&junkdata[0], sizeof(int)); // delayTimeL
	riffFile.Read(&junkdata[0], sizeof(int)); // delayTimeR
	riffFile.Read(&junkdata[0], sizeof(int)); // delayFeedbackL
	riffFile.Read(&junkdata[0], sizeof(int)); // delayFeedbackR

	riffFile.Read(&junkdata[0], sizeof(int)); // filterCutoff
	riffFile.Read(&junkdata[0], sizeof(int)); // filterResonance
	riffFile.Read(&junkdata[0], sizeof(int)); // filterLfospeed
	riffFile.Read(&junkdata[0], sizeof(int)); // filterLfoamp
	riffFile.Read(&junkdata[0], sizeof(int)); // filterLfophase
	riffFile.Read(&junkdata[0], sizeof(int)); // filterMode

	bool old;
	riffFile.Read(&old, sizeof(old)); // old format
	riffFile.Read(&_instance, sizeof(_instance)); // ovst.instance
	if ( old )
	{
		char mch;
		riffFile.Read(&mch,sizeof(mch));
		_program=0;
	}
	else
	{
...
bool VSTPlugin::LoadChunk(RiffFile &riffFile)
{
	if(_pEffect->flags & effFlagsProgramChunks)
	{
		long chunk_size;
...
//		while ( chunk_size != ck_sz && !riffFile.Eof())
//		{
//			riffFile.Skip(chunk_size);
//			riffFile.Read(&chunk_size,sizeof(long));
//		}
...
bool VSTPlugin::DescribeValue(int parameter,TCHAR* psTxt)
{
	if(instantiated)
	{
...
//			Dispatch(effGetParamName,parameter,0,par_name,0.0f);
			...
			{
				MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0019),_editName,NULL);
			}
			...
			{
				MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0020),_editName,NULL);
			}

//			_stprintf(psTxt,"%s:%s%s",par_name,par_display,par_label);
			
			_tcscpy(psTxt,
				CA2T(
				(boost::format("%s(%s)") % par_display % par_label).str().data()
				)
			);

			//CA2T _disp(par_display),_label(par_label);

			//_stprintf(psTxt,_T("%s ( %s )"),_label,_disp);
			//_tcscpy(psTxt,SF::tformat(_T("%s(%s)"),_disp,_label)
			return true;
		}
		else	_stprintf(psTxt,SF::CResourceString(IDS_MSG0000));//"ñ≥å¯ÇÃ NumParams íl"
	}
	else		_stprintf(psTxt,SF::CResourceString(IDS_MSG0001));//"ÉçÅ[ÉhÇµÇ‹ÇπÇÒÇ≈ÇµÇΩ"
...
int VSTPlugin::GetCurrentProgram()
{
	if(instantiated)
	{
		...
		{
			MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0021),_editName,NULL);
		}
		return ret;
	}
	else
...
void VSTPlugin::SetCurrentProgram(int prg)
{
	if(instantiated)
	{
		...
		{
			MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0022),_editName,NULL);
		}
	}
}
...
void VSTPlugin::SendMidi()
{
	if(instantiated && queue_size>0)
	{
		...
		{
			MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0023),_editName,NULL);
		}
	}
	queue_size=0;
}
...
long VSTPlugin::AudioMaster(AEffect *effect, long opcode, long index, long value, void *ptr, float opt)
{
	...
	ATLTRACE(SF::CResourceString(IDS_TRACE_MSG0004),(int)effect, opcode,index,value,(int)ptr,opt);
		...
		if (effect)
		{
			...
			if ( effect->user ) // ugly solution...
			{
				if (Global::pConfig->_RecordTweaks)  
				{
					...
					{
						CMainFrame::GetInstance().m_view.MousePatternTweakSlide(((VSTPlugin*)effect->user)->_macIndex, index, f2i(opt*VST_QUANTIZATION));
					}
					else
					{
						CMainFrame::GetInstance().m_view.MousePatternTweak(((VSTPlugin*)effect->user)->_macIndex, index, f2i(opt*VST_QUANTIZATION));
					}
				}
	...
	case audioMasterIdle:
		if (effect)
		{
			...
			{
				MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0024),SF::CResourceString(IDS_ERR_MSG0025),NULL);
			}
		}
...
*/
		...
		if ( (Global::pPlayer)->_playing) 
		{
			...
			if ( ((Master*)(Global::_pSong->pMachine(MASTER_INDEX)))->sampleCount == 0)
			{
				_timeInfo.flags |= kVstTransportChanged;
			}
		}
		
		if ( Global::_pSong->pMachine(MASTER_INDEX) ) // This happens on song loading with new fileformat.
		{
			_timeInfo.samplePos = ((Master*)(Global::_pSong->pMachine(MASTER_INDEX)))->sampleCount;
		}
...
/*			if ( (Global::pPlayer)->_playing) 
			...
			{*/
//				const double ppq = ((((Master*)(Global::_pSong->pMachine(MASTER_INDEX)))->sampleCount/ _timeInfo.sampleRate ) * (Global::pPlayer->bpm / 60.0f ));
//				_timeInfo.ppqPos =  ppq - 4*((int)ppq/4);
				_timeInfo.ppqPos = ((((Master*)(Global::_pSong->pMachine(MASTER_INDEX)))->sampleCount/ _timeInfo.sampleRate ) * (Global::pPlayer->bpm / 60.0f ));
//			}
		}
...
//		strcpy((char*)ptr,"Psycle");
	...
	case audioMasterUpdateDisplay:
		...
		if (effect)
		{
			...
			{
				MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0026),SF::CResourceString(IDS_ERR_MSG0025),NULL);
			}
		}
...
//			"acceptIOChanges",
	...
	case 	audioMasterSetTime:						
		ATLTRACE(_T("VST Master Dispatcher: Set Time\n"));
		break;
	case 	audioMasterGetNumAutomatableParameters:	
		ATLTRACE(_T("VST Master Dispatcher: GetNumAutPar\n"));
		break;
//	case 	audioMasterGetParameterQuantization:	
		ATLTRACE(_T("VST Master Dispatcher: ParamQuant\n"));
		break;
	case 	audioMasterIOChanged:					
		ATLTRACE(_T("VST Master Dispatcher:: IOchanged\n"));
		break;
//	case 	audioMasterSizeWindow:					
		ATLTRACE(_T("VST Master Dispatcher: Size Window\n"));
		break;
	case 	audioMasterGetInputLatency:				
		ATLTRACE(_T("VST Master Dispatcher: GetInLatency\n"));
		break;
	case 	audioMasterGetOutputLatency:			
		ATLTRACE(_T("VST Master Dispatcher: GetOutLatency\n"));
		break;
	case 	audioMasterGetPreviousPlug:				
		ATLTRACE(_T("VST Master Dispatcher: PrevPlug\n"));
		break;
	case 	audioMasterGetNextPlug:					
		ATLTRACE(_T("VST Master Dispatcher: NextPlug\n"));
		break;
	case 	audioMasterWillReplaceOrAccumulate:		
		ATLTRACE(_T("VST Master Dispatcher: WillReplace\n"));
		break;
	case 	audioMasterGetCurrentProcessLevel:		
		ATLTRACE(_T("VST Master Dispatcher: GetProcessLevel\n"));
		break;
	case 	audioMasterGetAutomationState:			
		ATLTRACE(_T("VST Master Dispatcher: GetAutState\n"));
		break;
	case 	audioMasterOfflineStart:				
		ATLTRACE(_T("VST Master Dispatcher: Offlinestart\n"));
		break;
	case 	audioMasterOfflineRead:					
		ATLTRACE(_T("VST Master Dispatcher: Offlineread\n"));
		break;
	case 	audioMasterOfflineWrite:				
		ATLTRACE(_T("VST Master Dispatcher: Offlinewrite\n"));
		break;
	case 	audioMasterOfflineGetCurrentPass:		
		ATLTRACE(_T("VST Master Dispatcher: OfflineGetcurrentpass\n"));
		break;
	case 	audioMasterOfflineGetCurrentMetaPass:	
		ATLTRACE(_T("VST Master Dispatcher: OfflineGetCurrentMetapass\n"));
		break;
	case 	audioMasterSetOutputSampleRate:			
		ATLTRACE(_T("VST Master Dispatcher: SetOutputsamplerate\n"));
		break;
	case 	audioMasterGetSpeakerArrangement:		
		ATLTRACE(_T("VST Master Dispatcher: Getspeaker\n"));
		break;
	case 	audioMasterSetIcon:						
		ATLTRACE(_T("VST Master Dispatcher: SetIcon\n"));
		break;
	case 	audioMasterOpenWindow:					
		ATLTRACE(_T("VST Master Dispatcher: OpenWindow\n"));
		break;
	case 	audioMasterCloseWindow:					
		ATLTRACE(_T("VST Master Dispatcher: CloseWindow\n"));
		break;
	case 	audioMasterGetDirectory:				
		ATLTRACE(_T("VST Master Dispatcher: GetDirectory\n"));
		break;
	default: 
		ATLTRACE(_T("VST Master Dispatcher: undefed: %d\n"),opcode);
		break;
	}	
...
VSTInstrument::VSTInstrument(int index)
{
	...
	_stprintf(_editName, SF::CResourceString(IDS_MSG0003));
	_program = 0;
...
void VSTInstrument::Tick(int channel,PatternEntry* pData)
{
	if(instantiated)
	{
		...
		else if (note < 120) // Noteon
		{
			...
			{
				if(pData->_volcmd == PatternCmd::VELOCITY){
					if ( pData->_inst== 0xFF) AddNoteOn(channel,note,pData->_volume/2); // should be 100, but previous host used 127
					else AddNoteOn(channel,note,pData->_volume/2,pData->_inst&0x0F);
				}
			}
		}
		...
		else if (note == cdefTweakS)
		{
			...
			if (i < MAX_TWS)
			{
				...
				TWSDelta[i] = ((TWSDestination[i] - TWSCurrent[i]) * TWEAK_SLIDE_SAMPLES)
...
void VSTInstrument::Work(int numSamples)
{
	...
	if (!_mute && instantiated)
	{
		if ( wantidle ) 
		{
			...
			{
				MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0024),_editName,NULL);
			}
		}
...
VSTFX::VSTFX(int index)
{
	...
	_type = MACH_VSTFX;
	_mode = MACHMODE_FX;
	_stprintf(_editName, _T("Vst2 Fx"));
	...
	if ((!_mute) && (!_stopped) && (!_bypass))
	{
		if (instantiated)
		{
			if ( wantidle ) 
			{
				...
				{
					MessageBox(NULL,SF::CResourceString(IDS_ERR_MSG0026),_editName,NULL);
				}
			}
