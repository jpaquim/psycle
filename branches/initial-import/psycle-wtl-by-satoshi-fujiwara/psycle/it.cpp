...
void ITHeader::Print(void) {
    ...
    _tprintf(_T("header ID: %s\n"), headerID);
    _tprintf(_T("songName: %s\n"), songName);
    _tprintf(_T("extra: %d\n"), extra);
    _tprintf(_T("numOrders: %d\n numIns: %d\nnumSamples: %d\nnumPatterns: %d\n"),
        numOrders, numIns, numSamples, numPatterns);
    
    _tprintf(_T("\ntrackerVersion: %d   formatVersion: %d\n"), trackerVersion, formatVersion);
    _tprintf(_T("flags: %d  special: %d\n"), flags, special);
    _tprintf(_T("\n"));

    _tprintf(_T("globalVolume: %d     mixVolume: %d\n"), globalVolume, mixVolume);
    _tprintf(_T("initialspeed: %d     initalTempo: %d\n"), initialSpeed, initialTempo);
    _tprintf(_T("separation: %d\n"), separation);
    _tprintf(_T("nullByte: %d\n\n"), nullByte);

    _tprintf(_T("messageLength: %d   messageOffset: %d\n"), messageLength, messageOffset);
    _tprintf(_T("reserved: %d\n"), reserved);
    _tprintf(_T("\n"));

    _tprintf(_T("channelPan: "));
    for (int i = 0; i < 64; i++)
    
    _tprintf(_T("%d "), channelPan[i]);
    _tprintf(_T("\n\n"));

    _tprintf(_T("channelVolume: "));
    for (i = 0; i < 64; i++)
        _tprintf(_T("%d "), channelVolume[i]);
    _tprintf(_T("\n\n"));

    _tprintf(_T("orders: "));
    for (i = 0; i < numOrders; i++)
            _tprintf(_T("%d "), orders[i]);
    _tprintf(_T("\n\n"));

    _tprintf(_T("instrumentOffset: "));
    for (i = 0; i < numIns; i++)
        _tprintf(_T("%d "), instrumentOffset[i]);
    _tprintf(_T("\n\n"));

    _tprintf(_T("sampleOffset: "));
    for (i = 0; i < numSamples; i++)
        _tprintf(_T("%d "), sampleOffset[i]);
    _tprintf(_T("\n\n"));

    _tprintf(_T("patternOffset: "));
    for (i = 0; i < numPatterns; i++)
        _tprintf(_T("%d "), patternOffset[i]);
    _tprintf(_T("\n\n"));
...
void ITInstrumentHeader::Print(void) {
    ...
    _tprintf(_T("ins. name: %s\n"), insName);
    _tprintf(_T("Header ID: %s\n"), headerID);
    _tprintf(_T("DOS filename: %s\n"), dosFileName);
    _tprintf(_T("nullbyte: %d\n"), nullByte);
    _tprintf(_T("NNA: %d  DCT: %d  DCA: %d\n"), newNoteAction, duplicateCheckType,
                duplicateCheckAction);

    _tprintf(_T("fadeOut: %d\n"), fadeOut);
    _tprintf(_T("pitchPanSeperation: %d   pitchPanCenter: %d\n"), 
                pitchPanSeperation, pitchPanCenter);
    _tprintf(_T("globalVolume: %d   defaultPan: %d\n"), globalVolume, defaultPan);
    _tprintf(_T("randomVolVariation: %d   randomPanVariation: %d\n"),
                randomVolumeVariation, randomPanningVariation);
    _tprintf(_T("trackerVersion: %d\n"), trackerVersion);
    _tprintf(_T("numSamples assoc: %d\n"), numSamples);
    _tprintf(_T("extraByte: %d  extraWord: %d\n"), extraByte, extraWord);
    _tprintf(_T("midiChannel: %d   midiProgram: %d   midiBank: %d\n"),
                midiChannel, midiProgram, midiBank);
    _tprintf(_T("\n"));

    _tprintf(_T("noteTable: "));
    for (int i = 0; i < 120; i++) {
        _tprintf(_T("%d-%d  "), noteTable[i].note, noteTable[i].sample);
    }
    _tprintf(_T("\n\n"));

    _tprintf(_T("volumeEnvelope - "));
    volumeEnvelope.Print();
    _tprintf(_T("panningEnvelope - "));
    panningEnvelope.Print();
    _tprintf(_T("pitchEnvelope - "));
    pitchEnvelope.Print();
...
void ITSampleHeader::Print(void) {
    ...
    _tprintf(_T("name: %s\n"), sampleName);
    _tprintf(_T("headerID: %s\n"), headerID);
    _tprintf(_T("dosFileName: %s\n"), dosFileName);
    _tprintf(_T("globalVolume: %d   defaultVolume: %d\n"), globalVolume, defaultVolume);
    _tprintf(_T("flags: %d   convert: %d\n"), flags, convert);
    _tprintf(_T("defaultPan: %d\n"), defaultPan);
    _tprintf(_T("length: %d"), length);
    _tprintf(_T("loopBegin: %d   loopEnd: %d   susLoopBegin: %d   susLoopEnd: %d\n"),
                loopBegin, loopEnd, sustainLoopBegin, sustainLoopEnd);
    _tprintf(_T("C5speed: %d\n"), C5speed);
    _tprintf(_T("sample data offset: %d\n"), sampleOffset);
    _tprintf(_T("vibSpeed: %d   vibDepth: %d   vibRate: %d   vibType: %d\n\n"),
...
void ITPattern::PackTo(ITPackedPattern& packed) {
    ...
    for (int frame = 0; frame < numRows; frame++) {
        ...
        for (int channel = 0; channel < numChannels; channel++) {
            ...
            // update _T("last") values for this channel
            if (maskVariable != 0)
...
void ITPattern::Print(void) {
    _tprintf(_T("numRows: %d\n"), numRows);
    _tprintf(_T("numChannels: %d\n\n"), numChannels);
