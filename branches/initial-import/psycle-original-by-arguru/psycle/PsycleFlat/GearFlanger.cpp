...
{	
	char buffer[16];
	sprintf(buffer,"%.2f%%",(float)machineRef->filterLfoamp*0.390625f);
	m_amp_label.SetWindowText(buffer);

	machineRef->UpdateFlanger();

	if(doit)
	machineRef->filterLfoamp=m_amp_slider.GetPos();
	*pResult = 0;
}
...
BOOL CGearFlanger::OnInitDialog() 
{
	...
	m_flanger_delay_slider.SetPos(machineRef->delayTimeL);

	m_amp_slider.SetRange(1,256);
	m_amp_slider.SetPos(machineRef->filterLfoamp);

	m_speed_slider.SetRange(0,32768);
	m_speed_slider.SetPos(machineRef->filterLfospeed);

	m_lf_slider.SetRange(-100,100);
	m_lf_slider.SetPos(machineRef->delayFeedbackL);

	m_rf_slider.SetRange(-100,100);
	m_rf_slider.SetPos(machineRef->delayFeedbackR);

	m_phase_slider.SetRange(0,256);
	m_phase_slider.SetPos(machineRef->filterLfophase);

	m_dry_slider.SetRange(-256,256);
	m_dry_slider.SetPos(machineRef->outDry);

	m_wet_slider.SetRange(-256,256);
	m_wet_slider.SetPos(machineRef->outWet);
...
{
	if(doit)
	machineRef->filterLfospeed=m_speed_slider.GetPos();
	
	char buffer[16];
	sprintf(buffer,"%d",machineRef->filterLfospeed);
...
{
	if(doit)
	machineRef->delayFeedbackL=m_lf_slider.GetPos();
...
{
	if(doit)
	machineRef->delayFeedbackR=m_rf_slider.GetPos();
...
{
	if(doit)
	machineRef->filterLfophase=m_phase_slider.GetPos();
	
	char buffer[16];
	sprintf(buffer,"%d",machineRef->filterLfophase);
...
{
	if(doit)
	machineRef->outDry=m_dry_slider.GetPos();
...
{
	if(doit)
	machineRef->outWet=m_wet_slider.GetPos();
...
{
	if(doit)
	machineRef->SetPreset(7,m_presetcombo.GetCurSel());
