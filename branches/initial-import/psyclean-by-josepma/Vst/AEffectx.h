...
// VstEvent
//-------------------------------------------------------------------------------------------------------
#define MAX_VST_EVENTS	64	// <- ADDED BY ARGURU!
...
struct VstEvents			// a block of events for the current audio block
{
	long numEvents;
	long reserved;			// zero
	VstEvent* events[MAX_VST_EVENTS];	// variable. <- [MAX_EVENTS] ADDED BY ARGURU!
};
...
enum
{
	...
	// realtime info
	audioMasterWillReplaceOrAccumulate,	// 22 - returns: 0: not supported, 1: replace, 2: accumulate
