...
// Properties
		...
		void pSampler(XMSampler * const p)
		{
			m_pSampler = p;
			m_FilterEnvelope.pSampler(p);
			m_AmplitudeEnvelope.pSampler(p);
			m_PanEnvelope.pSampler(p);
			m_PitchEnvelope.pSampler(p);
		}
		XMSampler * const pSampler() {	return m_pSampler; }
...
// Effect-Related Object Functions
		...
		void DelayedNote(NoteEvent data);

		// Tick n commands.
		void PanningSlide();
		void ChannelVolumeSlide();
		void NoteCut();
		void StopBackgroundNotes(NewNoteActions::nna_t action);
...
*/
	...
	XMSampler(MachineCallbacks* callbacks, Machine::id_type id, CoreSong* song);
	~XMSampler(){};
	...
	virtual void Tick(int channel, NoteEvent* pData);
...
/// properties
	...
	PlayerTimeInfo& getTimeInfo() { return callbacks->timeInfo(); }
	const PlayerTimeInfo& getTimeInfo() const { return callbacks->timeInfo(); }
