...
namespace psycle
{
	namespace audio_drivers
	{
		...
		{
			std::cout << "psycle: net audio: initializing\n";
			assert(!threadRunning_); ///\todo use proper synchronisation technic
			callback_ = callback;
		...
		{		
			std::cout << "psycle: net audio: " << (e ? "en" : "dis") << "abling\n";
			if (e && !threadRunning_ ) {
				if (!aud_) open();
				AuStatus status= AuBadValue;
				AuStartFlow(aud_, flow_, &status);
				if (status != AuSuccess) {
					std::cerr << "psycle: net audio: could not start the flow\n";
					return false;
				}
				killThread_ = false;
		...
		bool NetAudioOut::open()
		{
			...
			{
				std::cerr << "psycle: net audio: failed to open output: " << hostPort() << "\n";
				return false;
			}
			// 1.5)create a flow (workspace) for us.
			if (!(flow_ = AuCreateFlow(aud_, NULL)))
			{
				std::cerr << "psycle: net audio: failed to create flow\n";
				AuCloseServer(aud_);
				return false;
			}
			...
			for (int i=0; i < AuServerNumDevices(aud_); i++) {
				...
				if ((AuDeviceKind(dev) == AuComponentKindPhysicalOutput) &&
					AuDeviceNumTracks(dev) == 2) {
						...
						#if !defined NDEBUG
							std::cout << "psycle: net audio: device : " << i << ", id: " << device << "\n";
						#endif
						break;
					}
			}
			if ( device == AuNone ) 
			{
				std::cerr << "psycle: net audio: could not locate a stereo output device\n";
				AuCloseServer(aud_);
				return false;
			}
			...
			setSettings(sets);
			AuMakeElementImportClient(&nas_elements[0], settings().samplesPerSec(), toNasFormat(), 2, AuTrue, latency_, latency_/ 2, 0, NULL);
			AuMakeElementExportDevice(&nas_elements[1], 0, device_, settings().samplesPerSec(), AuUnlimitedSamples, 0, NULL);
			AuSetElements(aud_, flow_, AuTrue, 2, nas_elements, NULL);
			#if 0
			if (status != AuSuccess) {
				std::cerr << "psycle: net audio: cannot set audio elements: " << nas_error(aud_,status) << "\n";
				AuCloseServer(aud_);
				return false;
			}
			#endif

			std::cout << "psycle: net audio: opened: " << hostPort() << "\n";
			return true;
		}
		...
		std::string NetAudioOut::hostPort()
		{
			std::string nrv;
			{
				...
				if (!env) env = std::getenv("DISPLAY"); ///\todo can we have both the X server and NAS on the same port?
				if (env)
				{
					nrv = env;
					return nrv;
		...
		{
			threadRunning_ = true;
			std::cout << "xpsycle: net audio: device latency: " << latency_ << "\n";
			AuStatus state;
			while(!killThread_)
			{
				float const * input(callback_(callbackContext_, latency_));
				///\todo Stereo Hardcoded!!
				for (int i = 0; i < latency_*2; i++) {
					buf[i] = *input++;
				}
				AuWriteElement(aud_, flow_, 0, latencyInBytes(), (void*)buf, AuFalse, &state);
				// Dispose of waiting event messages as we don't implement event handling for simplicity reasons.
				AuSync(aud_, AuTrue);
				if (state == AuBadValue) {
				...
				if (state != AuSuccess )
				{
					std::cerr << "psycle: net audio: " << nas_error(aud_,state) << "\n";
				}
			}
			close();
		...
		static void NetAudioOut::writebuffer(GlobalDataPtr   g)
		{
			...
			AuWriteElement(g->remote.aud, g->remote.flow, 0, n, g->buf + g->out, AuFalse, NULL);
		...
		{
			///\todo hardcoded to 16bits for now.
