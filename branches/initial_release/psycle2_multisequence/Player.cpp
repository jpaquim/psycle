...
Player::Player()
{
	...
	{
		trackPatternID[i]	= -1; // actual play time track ID , Sequence EDIT can change actual track pattern ID 
		linePatternCounter[i] = 0; // every track pattern has it own line count!!!
	}
...
void Player::ExecuteLine(void)
{
	...
	_lineChanged = true;
	for (unsigned int track=0;  track < pSong->pSequencer->machineList.size(); track++ )
	{
		int rowNum = _globalLine;
		int trackID = pSong->pSequencer->machineList.at(track);
		SequenceTrack * pSeqTr = pSong->pSequencer->pSequenceData[trackID];
		if ( pSeqTr->pSequences[rowNum]->patternID != -1 ) 
		{
			trackPatternID[trackID]	= pSeqTr->pSequences[rowNum]->patternID;
			linePatternCounter[trackID] = 0;
		}

	}
	int backaTrack = pSong->activeTrack;
	for (unsigned seqTrack=0;  seqTrack < pSong->pSequencer->machineList.size(); seqTrack++ )
	{
		int trackID = pSong->pSequencer->machineList.at(seqTrack);
		if ( trackPatternID[trackID] == -1 ) continue;
		pSong->activeTrack = seqTrack;
		unsigned char* const plineOffset = pSong->_ptrackline(trackPatternID[trackID],0,linePatternCounter[trackID]);

		// Initial Loop. Check for Tracker Commands.

		for (int track=0; track<pSong->pSequencer->pSequenceData[trackID]->SONGTRACKS; track++)
		{
		...
		//
		for (track=0; track<pSong->pSequencer->pSequenceData[trackID]->SONGTRACKS; track++)
		{
...
void Player::AdvancePosition()
{
	...
	{
		int trackID = pSong->pSequencer->machineList.at(track);
		if ( trackPatternID[trackID] != -1 )
		{
			if ( linePatternCounter[trackID] < pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->patternLines[trackPatternID[trackID]])
				linePatternCounter[trackID]++;
		}
	}
	_lineCounter = linePatternCounter[pSong->pSequencer->machineList.at(pSong->activeTrack)];
	_globalLine++;
	if ( _globalLine == pSong->pSequencer->endLine ) 
	{
		_globalLine = pSong->pSequencer->startLine;
...
//	if (_lineCounter >= pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->patternLines[_playPattern])
//	{
//		//_lineCounter = 0;
//
//
