...
namespace SF {
	...
	void XMSongLoader::Load(string& fileName,Song& song,const bool fullopen)
	{

		if(!m_File.Open(fileName.c_str())){
			throw ISongLoader::Exception(string(SF::CResourceString(IDS_ERR_MSG0059)));
		};

		// check validity
		if(!IsValid()){
			throw ISongLoader::Exception(string(SF::CResourceString(IDS_ERR_MSG0060)));
		}
	...
	const long XMSongLoader::LoadPatterns(Song & song)
	{
		...
		if(pSongName==NULL)
			return 0;
		song.Name(string(CA2T(pSongName)));	
		song.Author(string(_T("")));
		song.Comment(string(SF::CResourceString(IDS_MSG0013)));
...
//						if(vol >= 0x60){						
	...
	const LONG XMSongLoader::LoadInstrument(XMSampler & sampler, LONG iStart, const int idx)
	{
		...
		char sInstrName[23];
		ZeroMemory(sInstrName,sizeof(sInstrName) * sizeof(char));
		...
		//std::string& _tmp1 = 
		sampler.Instrument(idx).Name(string(CA2T(sInstrName)));
