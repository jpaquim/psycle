...
{			
	...
	// check header
	bIsValid = (!stricmp(this->header.headerID, IT_HEADER));
...
long CFileIT::ImportPatterns(Song *s)
{
	...
	// get song name
	strcpy(s->Name, this->header.songName);	
	strcpy(s->Author,"");
	strcpy(s->Comment,"Impulse Tracker ”ºﬁ≠∞ŸÇ©ÇÁ≤›Œﬂ∞ƒÇµÇ‹ÇµÇΩÅB");

	for(int i=0; i<MAX_SONG_POSITIONS && i<256 && i<this->header.numOrders; i++)
	{
		if ( this->header.orders[i] < MAX_PATTERNS ) s->playOrder[i] = this->header.orders[i];
		else s->playOrder[i] = 0;
	}
	if ( this->header.numOrders > MAX_PATTERNS ) s->playLength = MAX_PATTERNS;
 	else s->playLength = this->header.numOrders;
	s->SONGTRACKS = 32;
	
	// tempo
	s->BeatsPerMin = 6 * this->header.initialTempo / this->header.initialSpeed;

	// instr count
	m_iInstrCnt = this->header.numIns;

	// get pattern data
	int nextPatStart = 0;
	for(int j=0;j<this->header.numPatterns&&nextPatStart>0;j++)
...
LONG CFileIT::ImportSinglePattern(Song *s, int patIdx, int iTracks)
{
	...
	int iNumRows = this->Patterns(patIdx).numRows;
	...
	else
	{
		...
		for(int row=0;row<iNumRows;row++)
			...
			{	
				...
				note = this->Patterns(patIdx).Note(col, row);
				instr = this->Patterns(patIdx).Instrument(col, row);
				//vol = this->Patterns(patIdx).VolPan(col, row);
				vol = 64;
				type = this->Patterns(patIdx).Command(col, row);
				param = this->Patterns(patIdx).CommandValue(col, row);
