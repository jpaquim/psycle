...
*/
CConfigDlg::CConfigDlg( ATL::_U_STRINGorID title , 
					   UINT uStartPage , HWND hWndParent ) :
CPropertySheetImpl<CConfigDlg> ( title, uStartPage, hWndParent )
{
	...
	configuration* pConfig,int dlgnum) 
{
	...
	_tcscpy(_skinDlg._pattern_fontface, pConfig->pattern_fontface);
	...
	_tcscpy(_skinDlg._pattern_header_skin, pConfig->pattern_header_skin);
	_skinDlg._pattern_font_flags = pConfig->pattern_font_flags;

	_tcscpy(_skinDlg._generator_fontface, pConfig->generator_fontface);
	_skinDlg._generator_font_point = pConfig->generator_font_point;
	_skinDlg._generator_font_flags = pConfig->generator_font_flags;

	_tcscpy(_skinDlg._effect_fontface, pConfig->effect_fontface);
	_skinDlg._effect_font_point = pConfig->effect_font_point;
	_skinDlg._effect_font_flags = pConfig->effect_font_flags;

	_tcscpy(_skinDlg._machine_skin, pConfig->machine_skin);

	_tcscpy(_skinDlg.szBmpBkgFilename, pConfig->szBmpBkgFilename);
	...
	TCHAR* ps = pConfig->GetInitialInstrumentDir();
	if (ps != NULL)
	{
		_tcscpy(_dirDlg._instPathBuf, ps);
	}
	ps = pConfig->GetInitialSongDir();
	if (ps != NULL)
	{
		_tcscpy(_dirDlg._songPathBuf, ps);
	}
	ps = pConfig->GetInitialPluginDir();
	if (ps != NULL)
	{
		_tcscpy(_dirDlg._pluginPathBuf, ps);
	}
	ps = pConfig->GetInitialVstDir();
	if (ps != NULL)
	{
		_tcscpy(_dirDlg._vstPathBuf, ps);
	}
	ps = pConfig->GetInitialSkinDir();
	if (ps != NULL)
	{
		_tcscpy(_dirDlg._skinPathBuf, ps);
		_tcscpy(_skinDlg._skinPathBuf, ps);
#if !defined(_PORTING)
		_tcscpy(_keyDlg._skinPathBuf, ps);
#endif
	}
	else
	{
...
{
	int retVal = CPropertySheetImpl<CConfigDlg>::DoModal();
	...
	if (retVal == IDOK)
	{
		...
		if (_tcscmp(_pConfig->pattern_header_skin, _skinDlg._pattern_header_skin))
		{
			// LOAD HEADER SKIN
			_tcscpy(_pConfig->pattern_header_skin, _skinDlg._pattern_header_skin);
#if !defined(_PORTING)
			if (_pConfig->Initialized() ) CMainFrame::GetInstance().m_view.LoadPatternHeaderSkin();
#endif
		}

		_pConfig->pattern_font_point = _skinDlg._pattern_font_point;
		_pConfig->pattern_font_flags = _skinDlg._pattern_font_flags;
		_tcscpy(_pConfig->pattern_fontface, _skinDlg._pattern_fontface);

		_pConfig->generator_font_point = _skinDlg._generator_font_point;
		_pConfig->generator_font_flags = _skinDlg._generator_font_flags;
		_tcscpy(_pConfig->generator_fontface, _skinDlg._generator_fontface);

		_pConfig->effect_font_point = _skinDlg._effect_font_point;
		_pConfig->effect_font_flags = _skinDlg._effect_font_flags;
		_tcscpy(_pConfig->effect_fontface, _skinDlg._effect_fontface);

		_pConfig->CreateFonts();

		if (_tcscmp(_pConfig->machine_skin, _skinDlg._machine_skin))
		{
			_tcscpy(_pConfig->machine_skin, _skinDlg._machine_skin);
			// LOAD HEADER SKIN
			if (_pConfig->Initialized() ) 
			{
#if !defined(_PORTING)
				CMainFrame::GetInstance().m_view.LoadMachineSkin();
#endif
			}
		}
		...
		if (_pConfig->bBmpBkg)
		{
			_tcscpy(_pConfig->szBmpBkgFilename, _skinDlg.szBmpBkgFilename);
			// LOAD HEADER SKIN
			if (_pConfig->Initialized() ) 
			{
#if !defined(_PORTING)
				CMainFrame::GetInstance().m_view.LoadMachineBackground();
#endif
			}
		}
		if (_pConfig->Initialized() ) 
		{
#if !defined(_PORTING)
			CMainFrame::GetInstance().m_view.RecalcMetrics();
#endif
		}
		...
		{
#if !defined(_PORTING)
//			_Module.get_m_pCreateWndList()->m_pThis
			CMainFrame::GetInstance().m_view.RecalculateColourGrid();
			CMainFrame::GetInstance().m_view.Repaint();
#endif
		}
	}
	return retVal;
