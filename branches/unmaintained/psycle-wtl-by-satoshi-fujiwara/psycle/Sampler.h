...
#define SAMPLER_MAX_POLYPHONY		32

#define SAMPLER_DEFAULT_POLYPHONY	32
...
public:
	...
	virtual bool LoadSpecificFileChunk(RiffFile& riffFile, int version)
	{
		...
		if (size)
		{
			...
			{
				int temp;
				riffFile.Read(&temp, sizeof(temp)); // numSubtracks
				_numVoices=temp;
				riffFile.Read(&temp, sizeof(temp)); // quality
	...
	virtual void SaveSpecificChunk(RiffFile& riffFile) 
	{
		...
		temp = _numVoices;
		riffFile.Write(&temp, sizeof(temp)); // numSubtracks
		...
		riffFile.Write(&temp, sizeof(temp)); // quality
	};
