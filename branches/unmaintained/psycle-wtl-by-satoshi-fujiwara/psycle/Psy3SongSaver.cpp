...
#endif
#include ".\psy3songsaver.h"
#include "ErrorString.h"

namespace SF {
	Psy3SongSaver::Psy3SongSaver(void)
	{
	}
	...
	void Psy3SongSaver::Save(RiffFile& riffFile,Song &song){
			// NEW FILE FORMAT!!!
...
//		::Sleep(1);
		...
		if (!riffFile.Close())
		{
			throw Exception((SF::tformat(SF::CResourceString(IDS_ERR_MSG0051)) % riffFile.szName).str());
		}
	}
	
	/// ファイル情報
	void Psy3SongSaver::SaveFileInfo(RiffFile& riffFile,Song& song)
	{
	...
	/// ソング情報
	void Psy3SongSaver::SaveSongInfo(RiffFile& riffFile,Song& song)
	{
		/*
		===================
		SONG INFO TEXT
...
//		UINT size = (song.Name().length() + song.Author().length() + song.Comment().length());
	...
	/// シーケンスデータ
	void Psy3SongSaver::SaveSequenceData(RiffFile& riffFile,Song& song){
		/*
		===================
		SEQUENCE DATA
		===================
	...
	/// パターンデータ
	void Psy3SongSaver::SavePatternData(RiffFile& riffFile,Song& song)
	{
		for (int i = 0; i < MAX_PATTERNS; i++)
		{
			// check every pattern for validity
	...
	/// マシンデータの読み込み
	void Psy3SongSaver::SaveMachineData(RiffFile& riffFile,Song& song)
	{
	...
	/// インストルメントデータの読み込み
	void Psy3SongSaver::SaveInstrumentData(RiffFile& riffFile,Song& song)
	{
