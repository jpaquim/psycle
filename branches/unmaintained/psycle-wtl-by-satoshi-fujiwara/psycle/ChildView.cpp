...
CChildView::~CChildView()
{
	...
	{
		char buf[100];
		sprintf(buf,"CChildView::~CChildView(). bmpDC (0x%.8X) を削除\n",(int)bmpDC);
		TRACE(buf);
		bmpDC->DeleteObject();
		delete bmpDC;
	}
...
void CChildView::OnPaint() 
{
	...
	if ( bmpDC == NULL && Global::pConfig->useDoubleBuffer ) // buffer creation
	{
		...
		char buf[100];
		sprintf(buf,"CChildView::OnPaint(). bmpDC を 0x%.8X にｲﾆｼｬﾗｲｽﾞ\n",(int)bmpDC);
		TRACE(buf);
	}
	else if ( bmpDC != NULL && !Global::pConfig->useDoubleBuffer ) // buffer deletion
	{
		char buf[100];
		sprintf(buf,"CChildView::OnPaint(). bmpDC (0x%.8X) を削除\n",(int)bmpDC);
		TRACE(buf);
		bmpDC->DeleteObject();
		delete bmpDC;
		bmpDC=NULL;
...
void CChildView::OnSize(UINT nType, int cx, int cy) 
{
	...
	{
		TRACE("CChildView::OnResize(). bmpDC を削除");
		bmpDC->DeleteObject();
		delete bmpDC;
		bmpDC=NULL;
	}
...
//	return false;
	...
	{
		if (MessageBox("上書き保存して構いませんか?","Song 保存",MB_YESNO) == IDYES)
		{
			...
			if (!file.Create(filepath.GetBuffer(1), true))
			{
				MessageBox("ﾌｧｲﾙ作成ｴﾗｰ!", "ｴﾗｰ!", MB_OK);
				return FALSE;
			}
			if (!_pSong->Save(&file))
			{
				MessageBox("ﾌｧｲﾙ保存ｴﾗｰ!", "ｴﾗｰ!", MB_OK);
				bResult = FALSE;
			}
			else 
			{
...
BOOL CChildView::CheckUnsavedSong(char* szTitle)
{
	...
	if (!bChecked)
	{
		if (Global::pConfig->bFileSaveReminders)
		{
			...
			sprintf(szText,"\'%s\' への変更を保存しますか?",Global::_pSong->fileName);
			...
			case IDYES:
				...
				{
					sprintf(szText,"\'%s\' への書き込みｴﾗｰです!!!",filepath);
...
{
	if (MessageBox("警告! Song を最後に保存して以降の変更のすべてを失います! 続けていいですか?","作業前の状態に戻す",MB_YESNO | MB_ICONEXCLAMATION) == IDYES)
	{
		if (Global::_pSong->_saved)
		{
			char buf[MAX_PATH];
...
void CChildView::OnRecordWav() 
{
	...
	{
		static char BASED_CODE szFilter[] = "WAV ﾌｧｲﾙ (*.wav)|*.wav|すべてのﾌｧｲﾙ (*.*)|*.*||";
...
void CChildView::NewMachine(int x, int y, int mac) 
{
	...
	if ((dlg.DoModal() == IDOK) && (dlg.Outputmachine >= 0))
	{
		...
		*/
		...
		{
			MessageBox("Machine の作成に失敗しました","ｴﾗｰ!",MB_OK);
		}
		else
		{
			if ( dlg.OutBus)
...
void CChildView::OnUpdateUndo(CCmdUI* pCmdUI)
{
	if(pUndoList) 
	{
		...
		default:
			...
			{
				pCmdUI->Enable(FALSE);
				pCmdUI->SetText("Pattern ｴﾃﾞｨﾀで元に戻す");
			}
			break;
		}
	}
...
void CChildView::OnUpdateRedo(CCmdUI* pCmdUI)
{
	if(pRedoList) 
	{
		...
		default:
			...
			{
				pCmdUI->Enable(FALSE);
				pCmdUI->SetText("Pattern ｴﾃﾞｨﾀでやり直し");
			}
			break;
		}
	}
...
void CChildView::OnFileImportXmfile() 
{
	...
	if (GetOpenFileName(&ofn)==TRUE)
	{
		...
		MessageBox(buffer,"XM ﾌｧｲﾙをｲﾝﾎﾟｰﾄしました",MB_OK);
...
{
	MessageBox("このｵﾌﾟｼｮﾝはまだ開発されていません","IT ﾌｧｲﾙをｲﾝﾎﾟｰﾄ",MB_OK);
...
void CChildView::FileLoadsongNamed(char* fName)
{
	...
	if (!file.Open(fName))
	{
		MessageBox("ﾌｧｲﾙを開くことが出来ませんでした。場所が正しいかﾁｪｯｸして下さい。", "ﾛｰﾄﾞｴﾗｰ", MB_OK);
		return;
	}
	editPosition = 0;
	_pSong->Load(&file);
...
//			pParentMain->UpdateComboIns(); PsyBarsUpdate calls UpdateComboGen that also calls UpdatecomboIns
	...
	if (Global::pConfig->bShowSongInfoOnLoad)
	{
		...
		MessageBox(buffer,"Song をﾛｰﾄﾞしました",MB_OK);
	}
}
		...
		if (!finder.IsDirectory())
		{
			...
			if (hbmMachineSkin)
			{
				...
				{	
					...
					{
						MessageBox("読み出しﾓｰﾄﾞでﾌｧｲﾙを開くことが出来ませんでした。操作は異常終了しました。","ﾌｧｲﾙｵｰﾌﾟﾝｴﾗｰ",MB_OK);
						return;
					}
					char buf[512];
