...
void CChildView::PreparePatternRefresh(int drawMode)
{
	...
	const int snt = _pSong->pSequencer->pSequenceData[trackID]->SONGTRACKS;
	const int plines = _pSong->pSequencer->pSequenceData[trackID]->patternLines[editPosition];
	...
	case DMPlayback: 
		{
				...
				(_pSong->playOrder[editPosition] == _pSong->playOrder[Global::pPlayer->_playPosition]))
			{
				if (pos != playpos)
				{
					...
					rect.left = 0;
					rect.right=CW;
					NewPatternDraw(0, _pSong->pSequencer->pSequenceData[trackID]->SONGTRACKS, pos, pos);
					updatePar |= DRAW_DATA;
					InvalidateRect(rect,false);
					if ((playpos >= 0) && (playpos != newplaypos))
					{
						...
						rect.left = 0;
						rect.right = CW;
						NewPatternDraw(0, _pSong->pSequencer->pSequenceData[trackID]->SONGTRACKS, playpos, playpos);
						updatePar |= DRAW_DATA;
						playpos =-1;
						InvalidateRect(rect,false);
					}
...
void CChildView::RecalculateColour(COLORREF* pDest, COLORREF source1, COLORREF source2)
{
	...
	int trackID = _pSong->pSequencer->machineList.at(_pSong->activeTrack);
	int len = _pSong->pSequencer->pSequenceData[trackID]->SONGTRACKS+1;
