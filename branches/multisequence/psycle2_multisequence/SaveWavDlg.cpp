...
void CSaveWavDlg::OnSavewave() 
{
	...
	if (m_savetracks.GetCheck())
	{
		...
		for (int i = 0; i < pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->SONGTRACKS; i++)
		{
			if (!_Muted[i])
			{
				count++;
				current = i;
				for (int j = 0; j < pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->SONGTRACKS; j++)
				{
					if (j != i)
					{
...
void CSaveWavDlg::SaveWav(char* file, int bits, int rate, int channelmode)
{
	...
	case 0:
		...
		//{
		//	j+=pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->patternLines[i];
		//}
		j = (pSong->pSequencer->endLine - pSong->pSequencer->startLine);
	...
	case 1:
		...
		m_progress.SetRange(0,pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->patternLines[pstart]);
	...
	case 2:
		...
		{
			pSong->playOrderSel[cont]=true;
			j+=pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->patternLines[pSong->playOrder[cont]];
		}
		m_progress.SetRange(0,j);
...
void CSaveWavDlg::SaveEnd()
{
	...
	if (m_savetracks.GetCheck())
	{
		...
		for (int i = current+1; i < pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->SONGTRACKS; i++)
		{
			if (!_Muted[i])
			{
				current = i;
...
void CSaveWavDlg::SaveTick()
{
	...
	{
		tickcont+=pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->patternLines[pSong->playOrder[i]];
	}
	if (lastpostick!= pPlayer->_playPosition ) 
	{
		tickcont+=pSong->pSequencer->pSequenceData[pSong->pSequencer->machineList.at(pSong->activeTrack)]->patternLines[pSong->playOrder[lastpostick]]-(lastlinetick+1)+pPlayer->_lineCounter;
	}
