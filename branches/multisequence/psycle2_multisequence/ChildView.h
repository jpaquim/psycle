...
{
public:
	const static int SEQ_ROWHEIGHT	= 17;
	const static int SEQ_YOFFSET	= 17;
	const static int SEQ_XOFFSET	= 60; // These two are variables, since changing from 
	const static int SEQ_ROWWIDTH	= 100; // horizontal to vertical, makes them different.

	int seqNumRows;
	int seqNumCols;

	int		rowStep;		// row sequnece step  1,2,4,8,16,32,64,128 ...
	int		row;			// row number
	int		track;			// machine track id 
	int		rowPatView;		// num of rows in pattern view
	int		colPatView;		// num of cols in pattern view
	int		selRowBeg;			// selection row
	int		selColBeg;			// selection col 
	int		selRowEnd;			// selection row
	int		selColEnd;			// selection col 
	...
	void SetNumRowsCols(int cw, int ch)
	{
		seqNumRows = ch/SEQ_ROWHEIGHT;
		seqNumCols = cw/SEQ_ROWWIDTH;
		if ( seqNumRows <= 0 ) seqNumRows = 1;
		if ( seqNumCols <= 0 ) seqNumCols = 1;
...
// KRISC 
public: 
	CSeqCursor seqCursor; // used in sequence cursor 
	CString oldFileName; 
...
//	void DrawMultiPatternData(CDC *devc,int tstart,int tend, int lstart, int lend);
	...
	void DrawSeqEditorKrisC(CDC *devc);
