...
void CChildView::OnTimer( UINT nIDEvent )
{
	if (nIDEvent == 31)
	{
		...
		if (Global::_pSong->_pMachine[MASTER_INDEX])
		{
			...
			float val = (float)((Master*)Global::_pSong->_pMachine[MASTER_INDEX])->_outDry;
...
void CChildView::OnFileSaveaudio() 
{
	...
	CSaveWavDlg dlg;
	if ( oldFileName != "" ) dlg.fileName = oldFileName;
	dlg.DoModal();
...
void CChildView::RedrawPatternSeq() 
{
	if (viewMode == VMSequence)
	{
		GetParent()->SetActiveWindow();
...
void CChildView::OnBarplay() 
{
	...
	Global::pPlayer->Start(editPosition,seqCursor.row * seqCursor.rowStep);
	pParentMain->StatusBarIdle();
}
...
void CChildView::OnBarplayFromStart() 
{
	...
	prevEditPosition=editPosition;
	Global::pPlayer->Start(0,_pSong->pSequencer->startLine);
	pParentMain->StatusBarIdle();
}
...
{
	CPatDlg dlg;
	int patNum = editPosition;
	int nlines = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[patNum];
	char name[32];
	strcpy(name,_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternName[patNum]);
	...
	if (dlg.DoModal() == IDOK)
	{
		if ( nlines != dlg.patLines )
		{
			...
			{
				strcpy(_pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternName[patNum],dlg.patName);
				pParentMain->StatusBarIdle();
				pParentMain->UpdateSequencer();
			}
			Repaint();
	...
	if ((dlg.DoModal() == IDOK) && (dlg.Outputmachine >= 0))
	{
		...
		*/
		...
		else
		{
			...
			if ( fb == 0x80 ) 
			{
				Global::_pSong->pSequencer->CreateMachineSequenceTrack(fb, buf);
			}
			else
			{
				if ( Global::_pSong->pSequencer->machineList.size() == 1 ) 
					if ( Global::_pSong->pSequencer->machineList.at(0) == 0x80 ) 
						Global::_pSong->pSequencer->machineList.clear();
				Global::_pSong->pSequencer->CreateMachineSequenceTrack(fb, buf);
			}
...
void CChildView::ShowSwingFillDlg(bool bTrackMode)
{
	...
	if (dlg.bGo)
	{
		...
		if (bTrackMode)
		{
			x = editcur.track;
			y = 0;
			ny = _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternLines[_ps()];
		}
		else
		{
...
//	file.Close(); <- load handles this
	...
	seqCursor.rowStep = _pSong->patStep;
	...
	CComboBox *cc=(CComboBox *)pParentMain->m_wndControl2.GetDlgItem(IDC_SEQCOMBO);
	char val[32];
	sprintf(val,"%d",_pSong->patStep);
	int nIndex = cc->SelectString(0,val);
...
void CChildView::DrawSeqEditorKrisC(CDC *devc)
{
	...
	////
	seqCursor.SetNumRowsCols(CW,CH);

	CRect rect;
	int curXoffset = seqCursor.SEQ_XOFFSET;
	int curYoffset = seqCursor.SEQ_YOFFSET;
...
/////////////////////////// Draw (Columns') Header
	...
	rect.top	= 0;
	rect.bottom = seqCursor.SEQ_YOFFSET;
	rect.left	= 0;
	rect.right	= CW;
	...
	TXT(devc,"Beat\\Gear",0,0,seqCursor.SEQ_XOFFSET-1,seqCursor.SEQ_YOFFSET-1);

	devc->SetBkColor(Global::pConfig->pvc_rowbeat);	// This affects TXT background

	curXoffset = seqCursor.SEQ_ROWWIDTH;
	curYoffset = seqCursor.SEQ_YOFFSET;
	for(int i = seqCursor.row - seqCursor.rowPatView; i < (seqCursor.seqNumRows-1) + seqCursor.row - seqCursor.rowPatView; i++)
	{
		if ((i%4) == 0) 
				devc->SetBkColor(Global::pConfig->pvc_rowbeat);	// This affects TXT background
			else 
				devc->SetBkColor(Global::pConfig->pvc_row);	// This affects TXT background
		if ((i%16) == 0) 
				devc->SetBkColor(Global::pConfig->pvc_row4beat);	// This affects TXT background


		sprintf(cbuffer, "%d", i * seqCursor.rowStep); // Row Step
		TXT(devc,cbuffer,0, curYoffset, seqCursor.SEQ_XOFFSET-1, seqCursor.SEQ_YOFFSET-1);
		curYoffset += seqCursor.SEQ_ROWHEIGHT;

	}
    
	curXoffset	= seqCursor.SEQ_XOFFSET;
	int count = seqCursor.track - seqCursor.colPatView;
	for(unsigned int i = seqCursor.track - seqCursor.colPatView; i < _pSong->pSequencer->machineList.size(); i++)
	{
		int tID = _pSong->pSequencer->machineList.at(i);
		if ( tID == 0x80 ) continue;
		SequenceTrack * pSeqTr = _pSong->pSequencer->pSequenceData[tID];
		sprintf(cbuffer, "%x:%s", count, pSeqTr->trackName);
		devc->SetBkColor(Global::pConfig->pvc_row);	// This affects TXT background
		TXT(devc, cbuffer, curXoffset, 0, seqCursor.SEQ_ROWWIDTH-1, seqCursor.SEQ_ROWHEIGHT-1);
		count++;
		...
		int rowStart = seqCursor.row - seqCursor.rowPatView - MAX_LINES;
		if ( rowStart < 0 ) rowStart = 0;
		for(int row = rowStart ; row < seqCursor.row - seqCursor.rowPatView; row++)
		{
			Sequence * pSeq = pSeqTr->pSequences[row * seqCursor.rowStep];
			if (pSeq->patternID != -1) 
			{
				patLen = pSeqTr->patternLines[pSeq->patternID];
			}
			if ( patLen > 0 ) 
				patLen -= seqCursor.rowStep;
		}


		for(int row = seqCursor.row - seqCursor.rowPatView; row < (seqCursor.seqNumRows-1) + seqCursor.row - seqCursor.rowPatView; row++)
		{
			if ((row%4) == 0) 
					devc->SetBkColor(Global::pConfig->pvc_rowbeat);	// This affects TXT background
				else 
			...
			bool select = false;
			if ( row >= seqCursor.selRowBeg && row <= seqCursor.selRowEnd &&  i >= (unsigned int)seqCursor.selColBeg && i <= (unsigned int)seqCursor.selColEnd) 
			{
					devc->SetBkColor(Global::pConfig->pvc_selection);	// This affects TXT background
					select = true;
			}
			if ( row <= seqCursor.selRowBeg && row >= seqCursor.selRowEnd &&  i <= (unsigned int)seqCursor.selColBeg && i >= (unsigned int)seqCursor.selColEnd) 
			{
			...
			Sequence * pSeq = pSeqTr->pSequences[row * seqCursor.rowStep];
			if (pSeq->patternID == -1) 
				sprintf(cbuffer, "...."); // Row Value
			else 
			{
				sprintf(cbuffer, "%X %s",pSeq->patternID , pSeqTr->patternName[pSeq->patternID] ); // Row Value
				patLen = pSeqTr->patternLines[pSeq->patternID];
			}
			// drawing track pattern length coloured box 
			if ( patLen > 0 ) 
			{
				if  ( select == false ) 
				{
					if ( patLen >= seqCursor.rowStep )
					{
						devc->SetBkColor(pSeqTr->patColor);
						TXT(devc,cbuffer,curXoffset, curYoffset, seqCursor.SEQ_ROWWIDTH-1, seqCursor.SEQ_ROWHEIGHT-1);
					}
					else
					{
						COLORREF prev = devc->GetBkColor();
						TXT(devc,"",curXoffset, curYoffset, seqCursor.SEQ_ROWWIDTH-1, seqCursor.SEQ_ROWHEIGHT-1);
						devc->SetBkColor(pSeqTr->patColor);
						float rowLines = (float)patLen / seqCursor.rowStep;
						int rowHeight = (int)((float)(seqCursor.SEQ_ROWHEIGHT-1) * rowLines);
						TXT(devc,"",curXoffset, curYoffset, seqCursor.SEQ_ROWWIDTH-1, rowHeight );
						CRect Rect;
						Rect.left = curXoffset ;
						Rect.top = curYoffset + rowHeight;
						Rect.right = curXoffset + seqCursor.SEQ_ROWWIDTH-1;
						Rect.bottom= curYoffset + seqCursor.SEQ_ROWHEIGHT-1;
						devc->SetBkColor(prev);
						devc->ExtTextOut(curXoffset + textLeftEdge, curYoffset, ETO_CLIPPED ,Rect,cbuffer,NULL);				
					}
				}
				else 
				{
	...
	if ( _pSong->GetMachineCount() > 0 ) 
	{
//////// Cursor
		rect.top	= seqCursor.SEQ_YOFFSET + seqCursor.rowPatView * seqCursor.SEQ_ROWHEIGHT;
		rect.left	= seqCursor.SEQ_XOFFSET + seqCursor.colPatView * seqCursor.SEQ_ROWWIDTH;
		rect.bottom = rect.top + seqCursor.SEQ_ROWHEIGHT-1;
		rect.right	= rect.left + seqCursor.SEQ_ROWWIDTH+1;
		int trackID = _pSong->pSequencer->machineList.at(seqCursor.track);
		SequenceTrack * pSeqTr = _pSong->pSequencer->pSequenceData[trackID];
		Sequence * pSeq = pSeqTr->pSequences[seqCursor.row * seqCursor.rowStep];
		strcpy(cbuffer,"....");
		if ( pSeq != NULL ) 
		{
			if ( pSeq->patternID != -1 ) 
			{	
				sprintf(cbuffer, "%X %s",pSeq->patternID , _pSong->pSequencer->pSequenceData[_pSong->pSequencer->machineList.at(_pSong->activeTrack)]->patternName[pSeq->patternID] ); // Row Value
			}
		}
		devc->SetBkColor(Global::pConfig->pvc_cursor);
		//devc->SetTextColor(Global::pConfig->pvc_fontCur);
		
		TXTFLAT(devc,cbuffer,rect.left,rect.top, seqCursor.SEQ_ROWWIDTH-1, seqCursor.SEQ_ROWHEIGHT-1);
	}
	...
	if ( Global::pPlayer->_playing )
	{
		int rowSel = (Global::pPlayer->_globalLine - (seqCursor.row - seqCursor.rowPatView) * seqCursor.rowStep) / seqCursor.rowStep;
		int rowVal = Global::pPlayer->_globalLine  / seqCursor.rowStep;
		devc->SetBkColor(Global::pConfig->pvc_playbar);
		devc->SetTextColor(Global::pConfig->pvc_font);
		sprintf(cbuffer, "%d", rowVal * seqCursor.rowStep); // Row Step
		TXT(devc,cbuffer,0,seqCursor.SEQ_YOFFSET + rowSel * seqCursor.SEQ_ROWHEIGHT, seqCursor.SEQ_XOFFSET-1, seqCursor.SEQ_YOFFSET-1);
	...
	for(int i = seqCursor.row - seqCursor.rowPatView; i < (seqCursor.seqNumRows-1) + seqCursor.row - seqCursor.rowPatView; i++)
	{
		...
		if ( i * seqCursor.rowStep == _pSong->pSequencer->startLine ) 
		{
			/////// Loop Range
			rect.top	= curYoffset;
		...
		if ( i * seqCursor.rowStep == _pSong->pSequencer->endLine ) 
		{
			/////// Loop Range
			rect.top	= curYoffset;
