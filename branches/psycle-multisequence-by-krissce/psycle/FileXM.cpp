...
long CFileXM::ImportPatterns(Song *s)
{
	...
	s->pSequencer->pSequenceData[s->activeTrack]->SONGTRACKS = iNoChannels;
...
LONG CFileXM::ImportSinglePattern(Song *s, LONG start, int patIdx, int iTracks)
{
	...
	if(patIdx<MAX_PATTERNS) s->pSequencer->pSequenceData[s->activeTrack]->patternLines[patIdx] = iNumRows;
