...
///\{
	...
	#define COMPILER__VERSION__MINOR 3
	/// compiler version, patch number.
	#define COMPILER__VERSION__PATCH 8
	...
	/// a compiler which does not generate code for a processor.
	/// (e.g. the eclipse cdt c++ indexer, the doxygen documentation compiler, resource compilers, etc ...)
	#define COMPILER__NO_PROCESSOR
	#undef COMPILER__NO_PROCESSOR // was just defined to insert documentation.
...
///\{
	/// prefers to use crossplatforms libraries when choice is available.
	...
	/// X Window System.
	/// On the apple macosx operating system,
	/// and on cygwin,
	/// we have to choose whether we want to use x window or not.
	...
	/// mswin, autodetected via _WIN32 and _WIN64.
...
// RC_INVOKED is defined by resource compilers (only relevant on microsoft's operating system).
	...
	#define COMPILER__VERSION__PATCH 2
	#if !defined __cplusplus
		#define __cplusplus
	#endif
...
#if defined COMPILER__ECLIPSE || COMPILER__DOXYGEN
	#define OPERATING_SYSTEM__CROSSPLATFORM
#endif
...
#if !defined PROCESSOR && !defined COMPILER__NO_PROCESSOR
...
#if !defined PROCESSOR__ENDIAN && !defined COMPILER__NO_PROCESSOR
...
///////////////
	...
	#define STRINGIZED(X) STRINGIZED_(X)
		...
		#define STRINGIZED_(X) #X
	#endif
	#define STRINGIZE(X) STRINGIZED(X)
#endif
...
// example: class riff_chunk attribute(packed, aligned(1)) { uint8[4] tag; uint32 size; uint8[0] data; }
...
// [bohan] this keyword is just syntactic sugar, so, we don't *need* it.
#if defined COMPILER__GNU
	// unsupported?
#elif defined COMPILER__MICROSOFT
	// super is a keyword
...
#if defined COMPILER__ECLIPSE || defined COMPILER__DOXYGEN
...
#if defined COMPILER__NO_PROCESSOR
	namespace compiler
	{
...
///////////
	...
	#if defined wchar_t
		#undef wchar_t
		typedef uint32 __identifier(wchar_t);
	#else
...
// \todo don't know why windef.h defines cdecl to nothing... because of this, functions with cdecl in their signature aren't considered to be of the same type as ones with __cdecl in their signature
// [bohan] ???
...
///////////////////////////////
	...
	#define LIBRARY__EXPORT "/dev/null" ///\todo /dev/null will fail on mingw outside of msys ... we need an empty file... or an already-included one... maybe #include __FILE__
	...
	#define LIBRARY__BEGIN  <operating_system/library/begin.h>
	#define LIBRARY__END    <operating_system/library/end.h>
	/**/#if 0
	...
	// see:
		#include <operating_system/library/export.h>
		#include <operating_system/library/import.h>
	...
	// Alternative way is to use:
		#include <operating_system/library/begin.h>
		#include <operating_system/library/end.h>
	// which attempt to be smarter by doing,
			#define class class attribute(dllexport)
	// or,
			#define class class attribute(dllimport)
...
// name pre-compiled headers
	...
	#if !defined HAS_CONFIG_H || defined ENABLE__PRE_COMPILED_HEADERS
