...
void animate(scene0 & scene, view & view, render & render) {
	this_thread::sleep_for(chrono::milliseconds(250)); // TODO calling start() and stop() right after may not work
	real z(0), t(0);
	while(true) {
		view.from(0, 0, z);
		//z -= 0.01;
		...
		render.process();
		this_thread::sleep_for(chrono::milliseconds(10)); // TODO hardcoded sleep
		render.wait();
