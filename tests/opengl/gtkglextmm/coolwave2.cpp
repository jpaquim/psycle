  ...
  for(i=0;i<grid;i++)
    {
      ...
      for(j=0;j<grid;j++) {
        #ifdef colorize
		  	glColor3f(
		  		0.1f + 1 * fabs(force[i][j]),
		  		0.1f + 10 * fabs(veloc[i][j]),
		  		0.9
		  	);
      	#endif
  ...
  for(i=0;i<grid;i++)
    {
      ...
      for(j=0;j<grid;j++) {
        #ifdef colorize
		  	glColor3f(
		  		0.1f + 1 * fabs(force[j][i]),
		  		0.1f + 10 * fabs(veloc[j][i]),
		  		0.9
		  	);
      	#endif
...
{
  GtkWidget *window = reinterpret_cast<GtkWidget*>(g_object_get_data(G_OBJECT(current), "window"));
  if (window)
    {
