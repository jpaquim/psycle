...
#include "microsoft_direct_sound.h"
#include "cstdint.h" /// \todo there's no such file in the current dir
//#include <cstdint>
namespace psycle
{
	namespace audio_drivers
	{
		...
		MicrosoftDirectSound * MicrosoftDirectSound::clone( ) const
		{
			return new MicrosoftDirectSound(*this);
		}

		void MicrosoftDirectSound::Initialize( AUDIODRIVERWORKFN pCallback, void * context )
		{
		...
		void MicrosoftDirectSound::Reset()
		{
			Stop();
		}
		...
		bool MicrosoftDirectSound::Start()
		{
			if(_running) return true; /// \todo use proper locking
			...
			format.nChannels = 2;///\todo hardcoded
			format.wBitsPerSample = 16;///\todo hardcoded
		...
		DWORD WINAPI MicrosoftDirectSound::PollerThread(void * pDirectSound)
		{
			MicrosoftDirectSound * pThis = reinterpret_cast<MicrosoftDirectSound*>( pDirectSound );
		...
		bool MicrosoftDirectSound::Stop()
		{
		...
		void MicrosoftDirectSound::DoBlocks()
		{
			int pos;
			HRESULT hr;
		...
		void MicrosoftDirectSound::ReadConfig()
		{
		...
		void MicrosoftDirectSound::WriteConfig()
		{
		}

		void MicrosoftDirectSound::Configure()
		{
		...
		int MicrosoftDirectSound::GetPlayPos()
		{
		...
		int MicrosoftDirectSound::GetWritePos()
		{
			if(!_running) return 0;
			int writePos;
		...
		bool MicrosoftDirectSound::Enable(bool e)
		{
			return e ? Start() : Stop();
		}

		void MicrosoftDirectSound::quantize(float *pin, int *piout, int c)
		{
			do
			{
				int r = static_cast<int>(pin[1]); /// \todo static_cast is slow on x87
				if (r < SHORT_MIN) r = SHORT_MIN;
				else if (r > SHORT_MAX) r = SHORT_MAX;

				int l = static_cast<int>(pin[0]); /// \todo static_cast is slow on x87
				if (l < SHORT_MIN) l = SHORT_MIN;
				else if (l > SHORT_MAX) l = SHORT_MAX;

				*piout++ = (r << 16) | static_cast<std::uint16_t>(l); /// \todo static_cast is slow on x87
				pin += 2;
			}
			while(--c);
		}
