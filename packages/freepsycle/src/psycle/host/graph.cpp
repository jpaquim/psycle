...
namespace psycle
{
	namespace host
	{
		...
		node::node(node::parent_type & parent, node::underlying_type & underlying, node::real const & x, node::real const & y)
		:
...
//				input_ports_.push_back(new ports::inputs::single(*this, single_input_port, x, y));
		...
		{
			if(multiple_input_port() && multiple_input_port()->underlying().name() == name) return multiple_input_port();
			for(single_input_ports_type::const_iterator i(single_input_ports().begin()) ; i != single_input_ports().end() ; ++i) if((**i).underlying().name() == name) return *i;
			if(loggers::trace()())
			{
				std::ostringstream s;
		...
		{
			for(output_ports_type::const_iterator i(output_ports().begin()) ; i != output_ports().end() ; ++i) if((**i).underlying().name() == name) return *i;
			if(loggers::trace()())
			{
				std::ostringstream s;
