...
namespace ngrs {
  ...
  GridLayout::GridLayout( ) : cols_(1),rows_(1), hgap_(0), vgap_(0)
  {
  }

  GridLayout::GridLayout(int cols, int rows)
    : Layout(), cols_(cols),rows_(rows), hgap_(0), vgap_(0)
  {
  }
  ...
  void GridLayout::align( VisualComponent * parent )
  {
  ...
  int GridLayout::preferredWidth( const VisualComponent * target ) const
  {
    return hgap_ + (findMaxColWidth() + hgap_) * cols_;
  }

  int GridLayout::preferredHeight( const VisualComponent * target ) const
  {
  ...
  int GridLayout::findMaxColWidth( ) const
  {
    int maxX = 0;
    std::vector<VisualComponent*>::const_iterator itr = parent()->visualComponents().begin();
    for (;itr < parent()->visualComponents().end(); itr++) {
  ...
  int GridLayout::findMaxRowHeight( ) const
  {
    int maxY = 0;
    std::vector<VisualComponent*>::const_iterator itr = parent()->visualComponents().begin();
  ...
  void GridLayout::setRows( int number )
  {
    rows_ = number;
  }

  void GridLayout::setColumns( int number )
  {
    cols_ = number;
  }

  void GridLayout::setHgap( int hgap )
  {
    hgap_ = hgap;
  }

  void GridLayout::setVgap( int vgap )
  {
    vgap_ = vgap;
  }
