...
namespace
{
  ...
  public:
    ...
    ///\{
  public:
    ///\todo [bohan] not sure Property<Class, GetValue, SetValue> is what's needed in the signal
    typedef sigslot::signal1<Property<Class, GetValue, SetValue>&> OnChangeSignal;
  ...
  public: // friend class PropertyMap::Map<AnyClass, AnyValue, AnyValue>
    Property() {}
  };
}
...
namespace ngrs {
  ...
  private:
    /// associative container that maps a Key to a Property
    template<typename Class, typename GetValue, typename SetValue> class Map : public std::map< Key, Property<Class, GetValue, SetValue> > {};
  ...
  public: // it's declared public but will actually probably only used by the object that owns the property map
    ...
    {
      map<Class, GetValue, SetValue>()[key] = Property<Class, GetValue, SetValue>(instance, getterMemberFunction, setterMemberFunction);
    }
    ...
    {
      map<Class, Value, Value>()[key] = Property<Class, Value, Value>(instance, 0, setterMemberFunction);
    }
    ///\}
  ...
  public:
    ...
    template<typename Class, typename GetValue, typename SetValue>
    typename Property<Class, GetValue, SetValue>::OnChangeSignal const & onChangeSignal(Key const & key) const throw(std::exception)
    {
      return find<Class, GetValue, SetValue>(key).onChangeSignal();
    }

    template<typename Class, typename GetValue, typename SetValue>
    typename Property<Class, GetValue, SetValue>::OnChangeSignal & onChangeSignal(Key const & key) throw(std::exception)
    {
      return find<Class, GetValue, SetValue>(key).onChangeSignal();
    }
  ...
  private:
    template<typename Class, typename GetValue, typename SetValue>
    Property<Class, GetValue, SetValue> const & find(Key const & key) const throw(std::exception)
    {
      return const_cast<PropertyMap&>(*this).find<Class, GetValue, SetValue>(key);
    }

    template<typename Class, typename GetValue, typename SetValue>
    Property<Class, GetValue, SetValue> & find(Key const & key) throw(std::exception)
    {
