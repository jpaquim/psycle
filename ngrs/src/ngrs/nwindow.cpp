...
{
  if ( selectedBase_ != NULL && selectedBase_->enabled() ) selectedBase_->onKeyPress(NKeyEvent(selectedBase_,keyEvent.buffer(),keyEvent.scancode(), keyEvent.shift() ));
}

void NWindow::onKeyRelease( const NKeyEvent & keyEvent )
{
  if (selectedBase_ != NULL && selectedBase_->enabled() ) selectedBase_->onKeyRelease(NKeyEvent(selectedBase_,keyEvent.buffer(),keyEvent.scancode(), keyEvent.shift() ));
}
