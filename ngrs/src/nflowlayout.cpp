...
void NFlowLayout::align( NVisualComponent * parent )
{
  ...
  std::vector<NRuntime*>::iterator start = itr;
  ...
  for (;itr < parent->components.end(); itr++) {
    ...
    if (child->visit(NVisualComponent::isVisualComponent)) {
       ...
       if ((xp + visualChild->preferredWidth() + hgap_ <= parent->clientWidth()) || (!lineBrk_) ) {
          ...
          std::vector<NRuntime*>::iterator itrH = start;
          for (;itrH < itr; itrH++) {
             NRuntime* child = *itrH;
             if (child->visit(NVisualComponent::isVisualComponent)) {
               ...
               //if (visual->name()=="test1") { printf("%d,%d,%d,%d\n",visualChild->preferredHeight(),ymax,yp,visual->top()); fflush(stdout); }
             }
          }
          yp = yp + ymax + vgap_;
  ...
  if (itr != start) {
    std::vector<NRuntime*>::iterator itrH = start;
    for (;itrH < parent->components.end() ; itrH++) {
       NRuntime* child = *itrH;
       if (child->visit(NVisualComponent::isVisualComponent)) {
         NVisualComponent* visual = (NVisualComponent*) child;
...
{
  //align();
  //if (maxY < parent()->minimumHeight()) return parent()->minimumHeight();
  return maxY;
}
...
void NFlowLayout::drawComponents( NVisualComponent * target, NGraphics* g, const NRect & repaintArea  )
{
  if (!lineBrk_) {
  ...
  } else NLayout::drawComponents(target,g,repaintArea);
}
