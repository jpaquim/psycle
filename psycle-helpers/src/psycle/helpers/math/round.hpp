...
int32_t inline rounded(Real x) {
	...
	int32_t inline rounded<>(long double ld) {
		return lrint<int32_t>(::roundl(ld));
	}

	template<> UNIVERSALIS__COMPILER__CONST
	int32_t inline rounded<>(double d) {
		return lrint<int32_t>(::round(d));
	}

	template<> UNIVERSALIS__COMPILER__CONST
	int32_t inline rounded<>(float f) {
		return lrint<int32_t>(::roundf(f));
	}
