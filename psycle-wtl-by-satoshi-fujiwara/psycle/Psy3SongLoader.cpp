...
//					MessageBox(NULL,_T("Info Seqment of File is from a newer version of psycle!"),NULL,NULL);
		...
		{
			TCHAR _buffer[256];
			song.Name(riffFile.ReadStringA2T(_buffer,Song::MAX_NAME_LEN));// Name 名前
			song.Author(riffFile.ReadStringA2T(_buffer,Song::MAX_AUTHOR_LEN));// Auhotr 作者
			song.Comment(riffFile.ReadStringA2T(_buffer,Song::MAX_COMMENT_LEN));// Comment コメント
		}
...
//					MessageBox(NULL,"Pattern section of File is from a newer version of psycle!",NULL,NULL);
		...
		else
		{
			...
			if (index < MAX_PATTERNS)
			{
				...
				temp = riffFile.ReadInt(); // num tracks per pattern // eventually this may be variable per pattern, like when we get multipattern
