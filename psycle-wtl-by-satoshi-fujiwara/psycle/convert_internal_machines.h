...
public:
	...
	Machine & redirect(const int & index, const int & type, RiffFile & riff) 
	{
		Plugin & plugin = * new Plugin(index);
		Machine * pointer_to_machine = &plugin;
		try
		{
			if(!plugin.LoadDll(const_cast<TCHAR*>((plugin_names()(type) + _T(".dll")).c_str())))
			{
			...
			assert(sizeof(int) == 4);
			char _edit_name[16];
			riff.Read(_edit_name, 16); /* sizeof machine._editName); */ 
			_edit_name[15] = 0;
			_tcscpy(machine._editName,CA2T(_edit_name));
	...
	void retweak(::Song & song) const
	{
		...
		{
			if(!song.pPatternData(pattern)) continue;
			PatternEntry * const lines 
				= reinterpret_cast<PatternEntry*>(song.pPatternData(pattern));
			for(int line = 0 ; line < song.PatternLines(pattern) ; ++line)
			{
				...
				for(int track = 0; track < song.SongTracks() ; ++track)
				{
					...
					{
						std::map<Machine * const, const int *>::const_iterator i 
							= machine_converted_from.find(song.pMachine(event._mach));
...
private:
	class Plugin_Names : private std::map<const int, const string *>
	{
	public:
		Plugin_Names()
		{
			(*this)[sine] = new string(_T("ring_modulator"));
			(*this)[distortion] = new string(_T("distortion"));
			(*this)[delay] = new string(_T("delay"));
			(*this)[filter_2_poles] = new string(_T("filter_2_poles"));
			(*this)[gainer] = new string(_T("gainer"));
			(*this)[flanger] = new string(_T("flanger"));
		}
		~Plugin_Names()
		{
			delete (*this)[sine];
		...
		const string & operator()(const int & type)
		{
			const_iterator i = find(type);
			if(i == end()) 
				throw string(SF::CResourceString(IDS_ERR_MSG0044));
			return *i->second;
		}
	};
		...
		case gainer:
			{
				...
				{
				case gain:
					value = scale::Exponential(maximum, exp(-4.0), exp(+4.0)).apply_inverse(value / 0x100);
					break;
				}
			}
			break;
