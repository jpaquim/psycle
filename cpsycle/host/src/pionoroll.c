...
{	
	...
	size = psy_ui_component_offsetsize_px(pianokeyboard_base(self));
	...
	psy_ui_Size* rv)
{
	*rv = psy_ui_size_make(psy_ui_value_makeew(10.0),
		psy_ui_value_makepx(self->keyboardstate->keyboardheightpx));
}
...
void pianogrid_ondraw(Pianogrid* self, psy_ui_Graphics* g)
{
	...
	pianogriddraw_init(&griddraw,
		...
		self->selection,
		psy_ui_component_offsetsize_px(pianogrid_base(self)),
...
{	
	...
	size = psy_ui_component_offsetsize_px(pianogrid_base(self));
	...
	PianogridTrackEvent* ev, psy_dsp_big_beat_t length)
{
	...
	corner = psy_ui_size_make_px(2, 2);
