...
void renderview_writedriverconfiguration(RenderView* self)
{
	...
	if (psy_audiodriver_configuration(self->fileoutdriver)) {
		...
		driversection = psy_property_find(self->driverconfigure,
			psy_property_key(psy_audiodriver_configuration(self->fileoutdriver)),
...
{		
	...
	current_wave_rec_dir = dirconfig_songs(&self->workspace->config.directories);
	psy_snprintf(outputpath, 4096, "%s%s%s", current_wave_rec_dir,
		psy_SLASHSTR, "Untitled.wav");
	psy_property_set_str(self->driverconfigure, "fileout.outputpath", outputpath);	
}
...
void renderview_configure_player_dither(RenderView* self)
{
	...
	if (psy_property_at_bool(self->driverconfigure, "fileout.dither.enable", FALSE) != FALSE) {
		...
		pdf = psy_property_at(self->driverconfigure, "fileout.dither.pdf", PSY_PROPERTY_TYPE_NONE);
		if (pdf) {
		...
		psy_audio_player_setdither(workspace_player(self->workspace), bitdepth,
			dither_pdf, dither_noiseshape);
...
void renderview_configure_player_record(RenderView* self)
{
	...
	psy_audio_sequence_setplayselection(&workspace_song(self->workspace)->sequence,
		&self->workspace->sequenceselection);
	psy_audio_sequencer_setplaymode(&workspace_player(self->workspace)->sequencer,
