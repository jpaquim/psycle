...
static void dev_starttimer(psy_ui_xt_CheckBoxImp* self, uintptr_t id, unsigned int interval) { self->xt_component_imp.imp.vtable->dev_starttimer(&self->xt_component_imp.imp, id, interval); }
static void dev_stoptimer(psy_ui_xt_CheckBoxImp* self, uintptr_t id) { self->xt_component_imp.imp.vtable->dev_stoptimer(&self->xt_component_imp.imp, id); }
