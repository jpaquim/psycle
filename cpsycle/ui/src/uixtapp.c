...
void psy_ui_xtapp_init(psy_ui_XtApp* self, void* instance)
{
	...
	printf("init xt\n");
	XtToolkitInitialize();
	printf("create app context xt\n");
	self->context = XtCreateApplicationContext();
	printf("open display xt\n");
	self->dpy = XtOpenDisplay(self->context, NULL,
	  NULL, NULL, 0, 0, &argc, 0);	
...
void psy_ui_xtapp_dispose(psy_ui_XtApp* self)
{
	psy_table_dispose(&self->selfmap);
	psy_table_dispose(&self->winidmap);
	XtDestroyApplicationContext(self->context);
...
int psy_ui_xtapp_run(psy_ui_XtApp* self) 
{
	XEvent xevent;
...
void psy_ui_xtapp_stop(psy_ui_XtApp* self)
{
//	PostQuitMessage(0);
}
