...
{	
	...
	if (self->state->dragthumb) {
		...
		size = psy_ui_component_offsetsize_px(&self->component);
		thumbsize = psy_ui_component_offsetsize_px(
			psy_ui_scrollbarthumb_base(&self->thumb));
		if (self->orientation == psy_ui_HORIZONTAL) {
...
double psy_ui_scrollbarpane_step(psy_ui_ScrollBarPane* self)
{
	...
	panesize = psy_ui_component_offsetsize_px(&self->component);
	size = psy_ui_component_offsetsize_px(psy_ui_scrollbarthumb_base(&self->thumb));
	if (self->orientation == psy_ui_HORIZONTAL) {
