...
// VTable Prototypes
static void dev_dispose(psy_ui_xt_OpenDialogImp*);
static int dev_execute(psy_ui_xt_OpenDialogImp*);
static const char* dev_filename(psy_ui_xt_OpenDialogImp*);

// VTable init
static psy_ui_OpenDialogImpVTable imp_vtable;
static int imp_vtable_initialized = 0;

static void imp_vtable_init(psy_ui_xt_OpenDialogImp* self)
{
...
void psy_ui_xt_opendialogimp_init(psy_ui_xt_OpenDialogImp* self, psy_ui_Component* parent)
{
...
void psy_ui_xt_opendialogimp_init_all(psy_ui_xt_OpenDialogImp* self,
...
// xt implementation method for psy_ui_OpenDialog
void dev_dispose(psy_ui_xt_OpenDialogImp* self)
{
	free(self->title);
	free(self->filter);
...
int dev_execute(psy_ui_xt_OpenDialogImp* self)
{
	int rv = 0;
	/*OPENFILENAME ofn;
	...
	*filename = '\0';
	...
	ofn.hwndOwner = 0; //  self->parent ? (HWND)psy_ui_xt_component_details(self->parent)->hwnd : (HWND)0;
	...
	if (app.main) {
		//InvalidateRect((HWND)psy_ui_xt_component_details(app.main)->hwnd, 0, FALSE);
		//UpdateWindow((HWND)psy_ui_xt_component_details(app.main)->hwnd);
	}
	if (rv) {
		free(self->filename);
...
const char* dev_filename(psy_ui_xt_OpenDialogImp* self)
{
