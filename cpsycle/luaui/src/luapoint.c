...
static int psy_luaui_point_create(lua_State* L);
static int psy_luaui_point_gc(lua_State* L);
static int psy_luaui_point_setxy(lua_State* L);
...
int psy_luaui_point_open(lua_State *L)
{
  ...
  luaL_newmetatable(L, psy_luaui_point_meta);
...
{  
  ...
  if (n == 1) {
	  ...
	  luaL_setmetatable(L, psy_luaui_point_meta);
  /*else if (n == 2) {
...
int psy_luaui_point_gc(lua_State* L)
{
	...
	ptr = *(psy_ui_Point**)luaL_checkudata(L, 1, psy_luaui_point_meta);	
	free(ptr);
	return 0;
}

int psy_luaui_point_setxy(lua_State* L)
{
	psy_ui_Point** ud;
	lua_Number x;
	lua_Number y;

	ud = (psy_ui_Point**)luaL_checkudata(L, 1, psy_luaui_point_meta);
	x = luaL_checknumber(L, 2);
	y = luaL_checknumber(L, 3);
	**ud = psy_ui_point_make(psy_ui_value_make_px(x),
