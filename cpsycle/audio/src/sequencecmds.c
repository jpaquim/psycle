...
{
	if (!(self->selection->editposition.track == 0 &&
			psy_audio_sequence_track_size(self->sequence,
				self->selection->editposition.track) < 2)) {
		psy_audio_sequenceselection_copy(self->selection, &self->restoreselection);
		self->patidx = psy_audio_sequence_patternindex(self->sequence, self->index);
		psy_audio_sequence_remove(self->sequence, self->index);
		self->index = psy_audio_orderindex_make(
			self->index.track,
			self->index.order > 0
