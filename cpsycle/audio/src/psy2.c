...
static void psy2loader_readinfo(PSY2Loader*);
...
void psy2loader_readinfo(PSY2Loader* self)
{
	int32_t m_beatspermin;
	int32_t m_linesperbeat;
	int32_t sampR;	

	psyfile_read(self->songfile->file, self->name_, 32);
	self->name_[32] = '\0';
	psyfile_read(self->songfile->file, self->author_, 32);
	self->author_[32] = '\0';
	psyfile_read(self->songfile->file, self->comments_, 128);
	self->comments_[128] = '\0';
	songproperties_init(&self->songproperties, self->name_,
		self->author_, self->comments_);
	psyfile_read(self->songfile->file, &m_beatspermin, sizeof m_beatspermin);
	psyfile_read(self->songfile->file, &sampR, sizeof sampR);
	self->songproperties.bpm = (psy_dsp_beat_t)m_beatspermin;
	if (sampR <= 0) {
		// Shouldn't happen but has happened.
		m_linesperbeat = 4;
	} else {
		// The old format assumes we output at 44100 samples/sec, so...	
		m_linesperbeat = (int32_t)(44100 * 60 / (sampR * m_beatspermin));
	}
	self->songproperties.tpb = 24;
	self->songproperties.extraticksperbeat = 0;
	self->songproperties.lpb = m_linesperbeat;
	psy_audio_song_setproperties(self->songfile->song, &self->songproperties);
}
	...
	for (i = 0; i < num; ++i) {
		...
		if (numlines > 0)
		{
			...
			for (c = 0; c < numlines; ++c) {
				...
				psyfile_read(self->songfile->file, pData, OLD_MAX_TRACKS * EVENT_SIZE);
				ptrack = pData;
...
void psy2loader_readvsts(PSY2Loader* self)
{
	...
	for (i = 0; i < OLD_MAX_PLUGINS; i++) {
		...
		if (valid) {
			psyfile_read(self->songfile->file, vstL[i].dllName,
				sizeof(vstL[i].dllName));
			psy_strlwr(vstL[i].dllName);
			psyfile_read(self->songfile->file, &(vstL[i].numpars), sizeof(int32_t));
			vstL[i].pars = malloc(sizeof(float) * vstL[i].numpars);
			for (int32_t c=0; c<vstL[i].numpars; c++) {
				psyfile_read(self->songfile->file, &(vstL[i].pars[c]), sizeof(float));
			}
		}
	}
}
...
void psy_audio_psy2_load(psy_audio_SongFile* songfile)
{
	...
	for (i = 0; i < 128; ++i) {
		if (_machineActive[i]) {
			...
			case MACH_PLUGIN:
			{
				...
				if (internalmachinesconvert_pluginname_exists(&converter, type, sDllName)) {
					...
					if (pMac[i]) {
						...
						psy_audio_machine_setslot(pMac[i], i);
			...
			psy_audio_machine_setposition(pMac[i], x, y);
		}
	}
	...
	// ::Sleep(1);
	if (psyfile_read(songfile->file, &psy2loader.busEffect[0],
		sizeof(psy2loader.busEffect)) == 0)
	{
		int j = 0;
		int i = 0;
		...
		for (i = 0; i < 64; i++)
		{
			if (psy2loader.busMachine[i] < 128 && psy2loader.busMachine[i] != 255) {
				invmach[psy2loader.busMachine[i]] = i;
			}
		}
	...
	for (i = 0; i < 64; i++)
	{
		...
		for (j = i + 1; j < 64; j++)
		{
			if (psy2loader.busMachine[i] == psy2loader.busMachine[j]) {
				psy2loader.busMachine[j] = 255;
			}
			if (psy2loader.busEffect[i] == psy2loader.busEffect[j]) {
				psy2loader.busEffect[j] = 255;
			}
		}
		for (j = 0; j < 64; j++)
		{
			if (psy2loader.busMachine[i] == psy2loader.busEffect[j]) {
				psy2loader.busEffect[j] = 255;
			}
		}
	}
	...
	for (i = 0; i < 64; i++)
	{
		if (psy2loader.busEffect[i])
		{
			if (psy2loader.busEffect[i] > 128 || (psy2loader.busEffect[i] < 128 && !_machineActive[psy2loader.busEffect[i]]))
				psy2loader.busEffect[i] = 255;
			// If there's a dummy, force it to be an effect
			else
			if (psy2loader.busEffect[i] < 128 && pMac[psy2loader.busEffect[i]] &&
				psy_audio_machine_type(pMac[psy2loader.busEffect[i]]) == MACH_DUMMY)
			{					
			...
			else if (psy2loader.busEffect[i] < 128 && pMac[psy2loader.busEffect[i]] &&
				psy_audio_machine_mode(pMac[psy2loader.busEffect[i]]) == MACHMODE_GENERATOR)
			{
				int k = 0;
				while (psy2loader.busEffect[k] != 255 && k < MAX_BUSES)
				{
					k++;
				}
				psy2loader.busMachine[k] = psy2loader.busEffect[i];
				psy2loader.busEffect[i] = 255;
			}
		}
		if (psy2loader.busMachine[i] != 255)
		{
			if (psy2loader.busMachine[i] > 128 || (psy2loader.busMachine[i] < 128 &&
					!_machineActive[psy2loader.busMachine[i]]))
				psy2loader.busMachine[i] = 255;
			// If there's a dummy, force it to be a Generator
			else if (psy_audio_machine_type(pMac[psy2loader.busMachine[i]]) == MACH_DUMMY)
			{
			...
			// This can't happen, but it is here for completeness
				...
				psy_audio_machine_mode(pMac[psy2loader.busMachine[i]]) != MACHMODE_GENERATOR)
			{
				int j = 0;
				while (psy2loader.busEffect[j] != 255 && j < MAX_BUSES)
				{
					j++;
				}
				psy2loader.busEffect[j] = psy2loader.busMachine[i];
				psy2loader.busMachine[i] = 255;
			}
		}
	}
		...
		
	{
		...
		for (i = 0; i < 64; i++)
		{
			if (psy2loader.busMachine[i] < 128 && psy2loader.busMachine[i] != 255) invmach[psy2loader.busMachine[i]] = i;
			if (psy2loader.busEffect[i] < 128 && psy2loader.busEffect[i] != 255) invmach[psy2loader.busEffect[i]] = i + 64;
		}
		...
		// todo samplerate
		internalmachineconverter_retweak_song(&converter, songfile->song, 44100.0);
		{
			//////////////////////////////////////////////////////////////////////////
