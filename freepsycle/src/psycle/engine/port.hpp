...
namespace psycle
{
	namespace engine
	{
		...
		typedef generic::port<typenames::typenames> port_base;
...
namespace psycle
{
	...
	{
		/// handles a stream of signal coming to or parting from a engine::node
		class UNIVERSALIS__COMPILER__DYNAMIC_LINK port : public named
		{
			protected:
				port(engine::node &, name_type const &, int const & channels = 0);
			public:
				virtual ~port() throw();
	
			public:
				/// the node which owns the port.
				engine::node inline & node() const { return node_; }
			private:
				engine::node        & node_;
			...
			///\{	
				public:
					/// assigns a buffer to this port (or unassigns if 0) only if the given buffer is different.
					void buffer(engine::buffer * const);
					/// the buffer to read or write data from or to (buffers are shared accross several ports).
					engine::buffer inline & buffer() const { return *buffer_; }
				protected:
					/// assigns a buffer to this port (or unassigns if 0) without checking if the given buffer is different.
					///\pre either the given buffer is 0, or else, this port has not yet been assigned a buffer.
					void virtual do_buffer(engine::buffer * const);
				//private:
					/// the buffer to read or write data from or to (buffers are shared accross several ports).
					engine::buffer * buffer_;
			///\}
