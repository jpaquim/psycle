...
namespace ngrs {
  ...
  void Label::init( )
  {
    ...
    // runtime
    //if (properties()) properties()->bind("text", *this, &Label::text, &Label::setText);
  }
  ...
  int Label::preferredHeight( ) const
  {
    if (!wbreak_) {
  ...
  int Label::preferredWidth( ) const
  {
    FontMetrics metrics(font());
    std::string::size_type i = 0;
    std::string::size_type start = 0;
  ...
  void Label::setMnemonic( char c )
  {
    mnemonic_ = c;
  }

  char Label::mnemonic() const {
    return mnemonic_;
  }
  ...
  int Label::vAlign( ) const
  {
    return valign_;
  }

  int Label::hAlign( ) const
  {
    return halign_;
  }

  void Label::setTextOrientation( int orientation )
  {
    orientation_ = orientation;
  }

  void Label::computeBreakPoints( )
  {
  ...
  int Label::findWidthMax(long width, const std::string & data, bool wbreak)
  {
    FontMetrics metrics(font());
...
// the class factories
extern "C" ngrs::Object* createLabel() {
  return new ngrs::Label();
}

extern "C" void destroyLabel( ngrs::Object* p ) {
  delete p;
}
