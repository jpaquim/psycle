...
namespace psycle {
	namespace host {
		...
		void Psy4Filter::onDetectFilterTag( const ngrs::XmlParser & parser, const std::string & tagName )
		{
...
//					else loggers::trace("Cursor still inside chunk, resyncing with chunk size.");
		...
		void Psy4Filter::onTagParse( const ngrs::XmlParser& parser, const std::string& tagName )
		{
			if ( tagName =="name" ) {
		...
		{
			std::string fileName = ngrs::File::extractFileNameFromPath(file_Name);
