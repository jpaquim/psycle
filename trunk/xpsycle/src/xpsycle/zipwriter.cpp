		...
		unsigned int flags)
{
	...
	f = malloc(sizeof(zipwriter_file));
	if (!f) {
		d->err = errno;
		return 0;
	}
	f->head = malloc(strlen(name)+30);
	if (!f->head) {
		d->err = errno;
		free(f);
		return 0;
	}
	f->head2 = malloc(strlen(name)+46);
	if (!f->head2) {
		d->err = errno;
		free(f->head);
		free(f);
