...
namespace psycle {
  namespace host {
      ...
      : NDialog(), fMac(mac)
    {
      ...
      ngrs::NPanel* btnBar = new ngrs::NPanel();
      ngrs::NGridLayout grid;
      grid.setVgap(5);
      grid.setHgap(5);
      btnBar->setLayout(grid);
      ngrs::NButton* btn;
      btnBar->add(new ngrs::NButton("Save",false));
      btnBar->add(new ngrs::NButton("Delete",false));
      btnBar->add(new ngrs::NButton("Import",false));
      btnBar->add(new ngrs::NButton("Export",false));
      btnBar->add(new ngrs::NCheckBox("Preview"));
      btn = new ngrs::NButton("Use");
      btn->setFlat(false);
      btn->clicked.connect(this,&PresetsDlg::onUseClicked);
      btnBar->add(btn);
      btnBar->add(new ngrs::NButton("Close",false));
      pane()->add(btnBar,ngrs::nAlRight);

      lBox = new ngrs::NListBox();
      pane()->add(lBox,ngrs::nAlClient);
        ...
        if (numpresets >= 0) {
          ...
          if (filenumpars == 1) {
            ...
            while (!f.Eof() ) {
              ...
              lBox->add(new ngrs::NItem(newPreset.name()));
            }
          }
        }
      } catch (const char * e) {
    ...
    void PresetsDlg::onUseClicked( ngrs::NButtonEvent * ev )
    {
      if (lBox->selIndex() != -1) {
        ngrs::NCustomItem* item = lBox->itemAt(lBox->selIndex());
