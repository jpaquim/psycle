...
namespace psycle
{
	namespace plugin
	{
		...
		void check(std::string const & lib_file_name, std::ostream & out) throw(std::exception)
		{
			...
			void * const lib(::dlopen(lib_file_name.c_str(), RTLD_NOW)); // RTLD_NOW to ensure the lib is fully usable
			if(!lib)
			{
				std::ostringstream s;
			...
			try
			{
				...
				#if defined DIVERSALIS__COMPILER__GNU && DIVERSALIS__COMPILER__VERSION__MAJOR >= 4 && 0 // disabled to make sure it works on untested compilers
