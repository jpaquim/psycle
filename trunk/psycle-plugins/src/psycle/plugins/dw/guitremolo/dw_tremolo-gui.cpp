...
// Every CMachineGuiParameter is built off of a CMachineParameter, even bitmaps and backgrounds.
// Also, an array of pointers to these classes is still needed in the plugin's CMachineInfo class.
...
enum
{
	prm_depth, prm_waveform, prm_gravity, prm_skew,
...
/// This map keeps track of all the parameter pointers for every instance of the plugin.
/// the key value is the index of the machine, which is sent to the constructor of the CMachineInterface class.
/// The constructor should save the index also, to erase it from the map when an instance is destroyed.
/// This is necessary because gui parameter classes contain instance-specific pointers and values,
/// which the old parameters didn't have. Therefore, plugin instances can't share the same parameters with eachother.
// std::map<int, CMachineGuiParameter**> InstMap;


/// This array should be filled with the CMachineParameters that the guiparameters are based off of.
/// The GuiParameters in the map are used for most things, but for now there are still some functions
/// that use these pointers to get parameter ranges and names and such.
...
// Note on the numcols variable and sizing in general:
// The size of the window is automatically calculated based on the positions of the gui parameters.
// The smallest size possible that will fit every parameter is chosen, in increments of 28 * 150 (the size of the old parameters).
// Currently, the numcols variable in the CMachineInfo class provides a starting point for the calculations,
// but that will probably be deemed unnecessary and may change.
...
protected:
	...
	float highCurve[MAPSIZE+2]; // later we scale from 0 (not 1) to MAPSIZE, which is why there's +1.. we make it +2 just in case (we'll init it to 1.0f later)
	int disp_counter;
...
/*
	...
	/// Since we're using functions the display class has that aren't in CMachineGuiParameter,
	/// this pointer is initialized as a casted alias for
	/// Params[prm_display]. We could've just casted it every time we call one.
	CMachineDisplay *Display;
...
mi::mi(int index)
{
	...
	for(int i(0);i<num_params;++i) Params[i]->value=&(Vals[i]);
...
void mi::Init()
{
	...
	// This coefficient changes the increment to lfopos..
	// Timing is defined by samples, so without it playing the same song in 22050 will halve the lfo speed.
	srMultiplier = 44100.f/(float)pCB->GetSamplingRate();
...
{
	float newsr = 44100.f/(float)pCB->GetSamplingRate(); // In case samprate is changed.. but this should probably go somewhere else.
	// It doesn't need to happen this frequently-- plus, it won't register until an actual song is playing.
	if(newsr != srMultiplier) srMultiplier = newsr; 
}
...
void mi::ParameterTweak(int par, int val)
{
	...
	if(par == prm_sync && val == 1) {
		...
		lfopos = 0;
		std::sprintf(Params[prm_sync]->lblString, "LFO Restarted!"); //Any guiparameter's label can be changed on the fly like this.
	}
}
...
void mi::Work(float *psamplesleft, float *psamplesright , int numsamples, int tracks)
{
	...
	do
	{
		...
		*psamplesright -= lfo[rtpos] * *psamplesright * volMult;
		...
		///\todo srMultiplier broke my plugin!# it worked great, until
		...
		if(Vals[prm_sync]==1 && --disp_counter < 0) {
			Vals[prm_sync]=0;
			std::sprintf(Params[prm_sync]->lblString, "Restart LFO");
		}
...
bool mi::DescribeValue(char* txt,int const param, int const value)
{
	...
	{
		case prm_depth:
			std::sprintf(txt," %.1f%% ", value/(float)(DEPTH_DIV/100.0f));
			return true;

		case prm_stereophase:
		case prm_skew:
			std::sprintf(txt, " %i deg. ", (int)((value - MAX_SKEW/2.0f) / (MAX_SKEW/360.0f)));
			return true;

		case prm_speed:
			std::sprintf(txt, " %.1f ms ", LFO_SIZE/(float)(value/(float)MAX_SPEED)/44.1f * LFO_SIZE/360.0f);								// please, don't ask me to explain how i derived
			return true;																																																																																				// this mess :) (i hate these conversions)

		case prm_gravity:
			if(Vals[prm_gravmode]==grav_updown) {
				if(value<100)
					std::sprintf(txt, " %i%% (upwards) ", value-100);
				else if(value>100)
					std::sprintf(txt, " %i%% (downwards) ", value-100);
				else
					std::sprintf(txt, " 0%% ");
			} else {
				if(value<100)
					std::sprintf(txt, " %i%% (outwards) ", value-100);
				else if(value>100)
					std::sprintf(txt, " %i%% (inwards) ", value-100);
				else
					std::sprintf(txt, " 0%% ");
			}
			return true;
		...
		case prm_gravmode:
			if(value == grav_updown) {
				std::sprintf(txt, " Up/Down ");
			} else if(value == grav_inout) {
				std::sprintf(txt, " In/Out ");
			} else {
				return false;
			}
			return true;
		...
		case prm_waveform:
			switch(value) {
				case lfo_sine:
					std::sprintf(txt, "sine");
					break;
				case lfo_tri:
					std::sprintf(txt, "triangle");
					break;
				case lfo_square:
					std::sprintf(txt, "square");
					break;
			}
			return true;
	}
...
void mi::fillLfo()
{
	...
	if(skew>LFO_SIZE||skew<1) skew = LFO_SIZE/2;
	// crashes psycle without this! on creation of the class, this function gets called before
	...
	switch(Vals[prm_waveform])
	{
		...
		case lfo_square:
			sqSlope = (int)(LFO_SIZE/80.0f); // 80.0f is a completely arbitrary constant, feel free to adjust to taste
	...
	// We *could* do all this with 0 gravity or a square wave, it just wouldn't change anything..
	if(Vals[prm_gravity]!=100 && Vals[prm_waveform]!=lfo_square)
	{
