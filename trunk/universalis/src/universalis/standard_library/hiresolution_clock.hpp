...
/******************************************************************************************/
	...
	BOOST_AUTO_TEST_CASE(std_hiresolution_clock_and_sleep_test)
	{
		...
		std::nanoseconds const t0(std::hiresolution_clock<std::nanoseconds>::universal_time());
		universalis::operating_system::threads::sleep(sleep_nanoseconds);
		double const ratio(double((std::hiresolution_clock<std::nanoseconds>::universal_time() - t0).get_count()) / sleep_nanoseconds.get_count());
		{
