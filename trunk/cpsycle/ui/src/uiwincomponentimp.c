...
void dev_sethorizontalscrollrange(psy_ui_win_ComponentImp* self, int min, int max)
{
	SCROLLINFO si;
	si.cbSize = sizeof(si);
	si.nMin = min;
...
void dev_setverticalscrollrange(psy_ui_win_ComponentImp* self, int min, int max)
{
	SCROLLINFO si;
	si.cbSize = sizeof(si);
	si.nMin = max(0, min);
