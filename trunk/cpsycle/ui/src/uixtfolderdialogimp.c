...
// VTable Prototypes
static void dev_dispose(psy_ui_xt_FolderDialogImp*);
static int dev_execute(psy_ui_xt_FolderDialogImp*);
static const char* dev_path(psy_ui_xt_FolderDialogImp*);

// VTable init
static psy_ui_FolderDialogImpVTable imp_vtable;
static int imp_vtable_initialized = 0;

static void imp_vtable_init(psy_ui_xt_FolderDialogImp* self)
{
...
void psy_ui_xt_folderdialogimp_init(psy_ui_xt_FolderDialogImp* self)
{
...
// xt implementation method for psy_ui_FolderDialog
void dev_dispose(psy_ui_xt_FolderDialogImp* self)
{
	free(self->title);
...
int dev_execute(psy_ui_xt_FolderDialogImp* self)
{
	...
	if (SHGetMalloc(&pMalloc) == NOERROR)
	{
		...
		//
		bi.hwndOwner = (HWND)0; // psy_ui_xt_component_details(self)->hwnd;
		bi.pidlRoot = NULL;
...
const char* dev_path(psy_ui_xt_FolderDialogImp* self)
{
	return self->path;
}
