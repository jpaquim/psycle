...
static void dev_move(psy_ui_win_CheckBoxImp* self, psy_ui_Point origin) { self->win_component_imp.imp.vtable->dev_move(&self->win_component_imp.imp, origin); }
...
static bool dev_inputprevented(const psy_ui_win_CheckBoxImp* self) { return self->win_component_imp.imp.vtable->dev_inputprevented(&self->win_component_imp.imp); }
