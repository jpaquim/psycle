...
// VTable Prototypes
static void dev_dispose(psy_ui_x11_OpenDialogImp*);
static int dev_execute(psy_ui_x11_OpenDialogImp*);
static const psy_Path* dev_path(const psy_ui_x11_OpenDialogImp*);

// VTable init
static psy_ui_OpenDialogImpVTable imp_vtable;
static int imp_vtable_initialized = 0;

static void imp_vtable_init(psy_ui_x11_OpenDialogImp* self)
{
	if (!imp_vtable_initialized) {
		imp_vtable = *self->imp.vtable;
		imp_vtable.dev_dispose = (psy_ui_fp_opendialogimp_dev_dispose)
			dev_dispose;
		imp_vtable.dev_execute = (psy_ui_fp_opendialogimp_dev_execute)
			dev_execute;
...
void psy_ui_x11_opendialogimp_init(psy_ui_x11_OpenDialogImp* self, psy_ui_Component* parent)
{
...
void psy_ui_x11_opendialogimp_init_all(psy_ui_x11_OpenDialogImp* self,
...
// x11 implementation method for psy_ui_OpenDialog
void dev_dispose(psy_ui_x11_OpenDialogImp* self)
{
	free(self->title);
	free(self->filter);
...
int dev_execute(psy_ui_x11_OpenDialogImp* self)
{
	int rv = 0;
	/*OPENFILENAME ofn;
...
const psy_Path* dev_path(const psy_ui_x11_OpenDialogImp* self)
{
	// return &self->path;
	return NULL;
}
