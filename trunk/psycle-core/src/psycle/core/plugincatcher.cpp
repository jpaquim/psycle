...
bool PluginFinderCache::loadCache(){
	...
	for (std::uint32_t i = 0; i < fileNumPlugs; i++)
	{
		...
		{ std::uint64_t filetime; file.Read(filetime); p.setFileTime((time_t )filetime); }
		...
		{ std::uint8_t allow; file.Read(allow); p.setAllow((bool)allow); }
		{ std::uint32_t role_int; file.Read(role_int); p.setRole(MachineRole::type(role_int)); }
...
bool PluginFinderCache::saveCache(){
	...
	for(std::uint32_t numHost = 1; hasHost(Hosts::type(numHost)); numHost++) {
	...
	for(std::uint32_t numHost = 1; hasHost(Hosts::type(numHost)); numHost++) {
		...
		while(iter != end(Hosts::type(numHost))) {
			...
			file.Write((std::uint64_t)info.fileTime());
			file.WriteString(info.error());
			file.Write((std::uint8_t)iter->first.host());
			file.Write(iter->first.index());
			file.Write((std::uint8_t)info.allow());
