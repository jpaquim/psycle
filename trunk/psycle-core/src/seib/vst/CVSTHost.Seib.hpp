...
namespace seib {
	namespace vst {
		...
		public:
			...
			virtual void crashed2(std::exception const & e) const {};
			...
			virtual bool DECLARE_VST_DEPRECATED(IsInputConnected)(int input) const { return true; } 
			virtual bool DECLARE_VST_DEPRECATED(IsOutputConnected)(int input) const { return true; }
		...
		public:
			...
			inline VstInt32 GetProgram() const { return Dispatch(effGetProgram); }
		...
		// VST 2.0
			...
			inline VstPlugCategory GetPlugCategory() const { return (VstPlugCategory)Dispatch(effGetPlugCategory); }
			...
			inline bool GetEffectName(char *ptr) const { return (bool)Dispatch(effGetEffectName, 0, 0, ptr); }
			// ptr is a string up to 256 chars + \0 delimiter
			inline bool DECLARE_VST_DEPRECATED(GetErrorText)(char *ptr) const { return (bool)Dispatch(effGetErrorText, 0, 0, ptr); }
			// ptr is a string up to kVstMaxVendorStrLen chars + \0 delimiter
			inline bool GetVendorString(char *ptr) const { return (bool)Dispatch(effGetVendorString, 0, 0, ptr); }
			// ptr is a string up to kVstMaxProductStrLen chars + \0 delimiter
			inline bool GetProductString(char *ptr) const { return (bool)Dispatch(effGetProductString, 0, 0, ptr); }
			inline long GetVendorVersion() const { return Dispatch(effGetVendorVersion); }
			inline long VendorSpecific(long index, long value, void *ptr, float opt) const { return Dispatch(effVendorSpecific, index, value, ptr, opt); }
			//returns 0 -> don't know, 1 -> yes, -1 -> no. Use the PlugCanDo's strings.
			inline long CanDoInt(const char *ptr) const { return Dispatch(effCanDo, 0, 0, (void *)ptr); }
			inline bool CanDo(const char *ptr) const { return (Dispatch(effCanDo, 0, 0, (void *)ptr)>0); }
			...
			inline long GetVstVersion() const { VstInt32 v=Dispatch(effGetVstVersion); if (v > 1000) return v; else return (v)?2000:1000; }
		// VST 2.1 extensions
			// Seems something related to MAC ( to be used with editkey )
		...
		public:
			...
			virtual long OnGetHostVendorVersion() const { return 1612; } // 1.16l
			virtual long OnHostVendorSpecific(CEffect &pEffect, long lArg1, long lArg2, void* ptrArg, float floatArg) const { return 0; }
			virtual long OnGetVSTVersion() const { return kVstVersion; }
			...
			virtual long OnCurrentId(CEffect &pEffect) const { return pEffect.uniqueId(); }
			// This callback forces an inmediate "EditIdle()" call for the plugins.
			virtual void OnIdle(CEffect &pEffect);
			virtual bool DECLARE_VST_DEPRECATED(OnInputConnected)(CEffect &pEffect, long input) const { return pEffect.IsInputConnected(input); }
			virtual bool DECLARE_VST_DEPRECATED(OnOutputConnected)(CEffect &pEffect, long output) const { return pEffect.IsOutputConnected(output); }
