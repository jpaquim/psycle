...
namespace psycle
{
	namespace host
	{
		...
		bool Configuration::Read()
		{
			...
			if(reg.OpenRootKey(HKEY_CURRENT_USER, PSYCLE__PATH__REGISTRY__ROOT) != ERROR_SUCCESS) return false;
			bool existed(true);
			...
			if(result != ERROR_SUCCESS)
			{
				...
				{
					if(reg.CreateKey("configuration") != ERROR_SUCCESS) return false;
					existed = false;
				}
				else return false;
			}
		...
		void Configuration::Write()
		{
			...
			if(reg.OpenKey("configuration") != ERROR_SUCCESS)
			{
				if(reg.CreateKey("configuration") != ERROR_SUCCESS)
				{
					Error("Unable to write configuration to the registry");
					return;
				}
