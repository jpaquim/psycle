...
//	Machine & redirect(const int & index, const int & type, RiffFile & riff) throw(std::string)
	...
	void retweak(Song & song) const
	{
		...
		{
			if(!song.pSequencer->pSequenceData[song.activeTrack]->ppPatternData[pattern]) continue;
			PatternEntry * const lines = reinterpret_cast<PatternEntry*>(song.pSequencer->pSequenceData[song.activeTrack]->ppPatternData[pattern]);
			for(int line = 0 ; line < song.pSequencer->pSequenceData[song.activeTrack]->patternLines[pattern] ; ++line)
			{
				PatternEntry * events = lines + line * MAX_TRACKS;
				for(int track = 0; track < song.pSequencer->pSequenceData[song.activeTrack]->SONGTRACKS ; ++track)
				{
		...
		case gainer:
			{
				...
				{
				case gain:
					value = scale::Exponential(maximum, exp((double)-4), exp((double)+4)).apply_inverse(value / 0x100);
					break;
				}
			}
			break;
