		...
		if ((_mode == MACHMODE_GENERATOR) || (!_bypass && !_stopped))
		{
			...
			while (ns)
			{
				...
				if (nextevent > ns)
				{
					...
					for (i=0; i < Global::_pSong->pSequencer->pSequenceData[Global::_pSong->pSequencer->machineList.at(Global::_pSong->activeTrack)]->SONGTRACKS; i++)
					{
						// come back to this
						if (TriggerDelay[i]._cmd)
						{
					...
					_pInterface->Work(_pSamplesL+us, _pSamplesR+us, ns, Global::_pSong->pSequencer->pSequenceData[Global::_pSong->pSequencer->machineList.at(Global::_pSong->activeTrack)]->SONGTRACKS);
				...
				else
				{
					...
					{
						ns -= nextevent;
						_pInterface->Work(_pSamplesL+us, _pSamplesR+us, nextevent, Global::_pSong->pSequencer->pSequenceData[Global::_pSong->pSequencer->machineList.at(Global::_pSong->activeTrack)]->SONGTRACKS);
						us += nextevent;
					}
					if (TWSActive)
					{
