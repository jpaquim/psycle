...
// int SEQ_ROWWIDTH = 100;  // horizontal to vertical, makes them different.
...
//
//						int psize=(_pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternLines[_pSong->playOrder[j]]*SEQ_ROWWIDTH)/(_pSong->_ticksPerBeat*seqSteps);
//						rect.left=curXoffset;
...
//						sprintf(cbuffer,"%.2d:%s",_pSong->playOrder[j],_pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternName[_pSong->playOrder[j]]);
...
//		for (i=0;k<seqTick;i++)
//		{
//			k+=_pSong->pSequencer->pSequenceData[_pSong->activeTrack]->patternLines[_pSong->playOrder[i]]/(seqSteps*_pSong->_ticksPerBeat);
//		}
