...
*/					__m128 zeroval = _mm_set_ps1(0.0f);
			...
			#if 0
				///\todo doc
				inline int GetMaxVolAccurate(float *pSamplesL, float *pSamplesR, int numSamples)
				{
					return f2i(GetMaxVSTVolAccurate(pSamplesL,pSamplesR,numSamples));
				}

				///\todo doc
				inline float GetMaxVSTVolAccurate(float *pSamplesL, float *pSamplesR, int numSamples)
				{
					--pSamplesL;
					--pSamplesR;
			...
			///\todo doc
			///\param amplitude normalized to 1.0f.
			inline float dB(float amplitude)
			{
			...
			class Cubic : public Resampler
			{
				...
				private:
					...
					///
					/// note: even with this optimization, interpolating the sinc table roughly doubles the cpu usage on some machines.
